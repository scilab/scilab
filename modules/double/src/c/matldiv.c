...
int matldiv()
{
	...
	else if(iRows1 * iCols1 != 1)
	{
		...
		{// A and B are both real
			...
			iRet = iLeftDivisonOfRealMatrix(pReal1, iRows1, iCols1, pReal2, iRows2, iCols2, pReturnReal, iCols1, iCols2);
		...
		{/* A real, B complex : complexify A */
			...
			iRet = iLeftDivisonOfComplexMatrix(
	...
	{// Real \ Matrix 
		...
		{// Real \ Complex -> Complex / Real
			iErr = iRightDivisionComplexMatrixByRealMatrix(
				pReal2,			pImg2,		1, 
				pReal1,						0,
				pReturnReal,	pReturnImg, 1, iRows2 * iCols2);
		}
