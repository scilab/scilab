				...
				double *_pdblRealOut,	double *_pdblImgOut,	int _iIncOut, int _iSize)
{
	...
	for(iIndex = 0 ; iIndex < _iSize ; iIndex++)
	{
		iErr2 = iRightDivisonComplexByReal(_pdblReal1[iIndex1], _pdblImg1[iIndex1], _pdblReal2[iIndex2], &_pdblRealOut[iIndexOut], &_pdblImgOut[iIndexOut]);
		if(iErr2 != 0)
			iErr = iIndex;
		iIndexOut	+= _iIncOut;
		iIndex1		+= _iInc1;
			...
			double *_pdblRealOut,	int _iRowsOut,	int _iColsOut)
{
	...
	//Dans mon cas on alloue un tableau de la taille restante dans la stack.
	...
	if(_iRows1 == _iCols1)
	{
		...
		if(iInfo == 0)
		{
			...
			if(dblRcond > dsqrts(dblEps))
			{
				...
				cNorm	= 'F';
				C2F(zlacpy)(&cNorm, &_iCols1, &_iCols2, pXb, &_iCols1, _pdblRealOut, &_iCols1);
				return 0;
			}
			else
			{
