...
if test "$rest" = "yes"; then
  ...
  if test -n "$nos"; then
     if test -n "$now"; then
      $SCI/bin/scilex -ns -nw $start_file $arguments
     else
      $SCI/bin/zterm -e $SCI/bin/scilex -ns $display $start_file $arguments&
     fi
  else
     if test -n "$now"; then
      $SCI/bin/geci -local $SCI/bin/scilex -nw $start_file $arguments
     else
      $SCI/bin/zterm -e $SCI/bin/geci -local $SCI/bin/scilex $start_file $arguments
     fi
  fi    
