      subroutine mpdspis(ma,na,a,nela,inda,i,ni,j,nj,
c     Copyright INRIA
     $     mb,nb,b,mr,nr,r,nelr,indr,ierr)
c     insert a full submatrix in a sparse matrix
c     modified to work in tropical algebras/sg
c!
      ...
      external findl
c
      write(*,*)'PCZERO in mpdspis=',PCZERO
      nelmx=nelr
      ierr=0
      mr=ni
      nr=nj
...
c     this line is not modified
               if(l.le.ma) then
                  ...
                  call dcopy(indr(l),a(ja),1,r(jr),1)
                  jr=jr+indr(l)
                  ja=ja+indr(l)
               else
                  indr(l)=0
...
c     *     the a(l,:) is not modified
               if(l.le.ma) then
                  ...
                  call dcopy(indr(l),a(ja+ja1),1,r(jr),1)
                  jr=jr+indr(l)
                  ja1=ja1+indr(l)
               else
                  indr(l)=0
