...
c     .     Initialize tvec
         ...
         if(ntvec.gt.0) then
            if(funtyp(kfun).ne.-1)  call dset(ntvec,told-1.0d0,tvec,1)
c     
            flag=3
...
C     !                 event conflict
 ...
 60   continue
      ...
      subroutine ddoitdassl(neq,x,xptr,z,zptr,iz,izptr,told,tf
     $     ,tevts,evtspt,nevts,pointi,inpptr,inplnk,outptr
     $     ,outlnk,lnkptr,clkptr,ordptr,nptr
     $     ,ordclk,nordcl,cord,iord,niord,oord,zord,critev,
     $     rpar,rpptr,ipar
...
c     
      ...
      pointi=evtspt(keve)
      evtspt(keve)=-1
c     TO HANDLE DISCONTINUITIES CAUSED BY EVENT
      if(critev(keve).eq.1) then
         hot=.false.
      endif
c     
      nord=ordptr(keve+1)-ordptr(keve)
...
C     !                 event conflict
 ...
 60   continue
      if (urg.gt.0)  then
 43      call edoitdassl(neq,x,xptr,z,zptr,iz,izptr,told,tf
     $        ,tevts,evtspt,nevts,pointi,inpptr,inplnk,outptr
     $        ,outlnk,lnkptr,clkptr,ordptr,nptr
     $        ,ordclk,nordcl,cord,iord,niord,oord,zord,critev,
     $        rpar,rpptr,ipar
...
C     If continuous state jumps, do cold restart
 ...
 61   continue
      ...
      subroutine edoitdassl(neq,x,xptr,z,zptr,iz,izptr,told,tf
     $     ,tevts,evtspt,nevts,pointi,inpptr,inplnk,outptr
     $     ,outlnk,lnkptr,clkptr,ordptr,nptr
     $     ,ordclk,nordcl,cord,iord,niord,oord,zord,critev,
     $     rpar,rpptr,ipar
