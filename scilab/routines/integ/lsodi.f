...
c     double precision atol, rwork, rtol, t, tout, y, ydoti
...
c     r(1) = -0.040d+0*y(1) + 1.0d+4*y(2)*y(3) - s(1)
c     r(2) =  0.040d+0*y(1) - 1.0d+4*y(2)*y(3) - 3.0d+7*y(2)*y(2) - s(2)
...
c     p(1,1) = p(1,1) + 1.0d+0
c     p(2,2) = p(2,2) + 1.0d+0
c     return
c     end
c
...
c
c iv.  description of two subroutines in the lsodi package, either of
...
c          (dimensioned in the subroutines) and y has length exceeding
c          neq(1). however, these subroutines should not alter
...
c  prepji   computes and preprocesses the jacobian matrix j = df/dy
...
c max, min, dble, abs, max, min, mod, sign, sqrt, and write.
c
...
c
c!
c this is the may 9, 1983 version of lsodi.
...
c-----------------------------------------------------------------------
 ...
 145  if (tol .gt. 0.0d+0) go to 160
      atoli = atol(1)
      do 150 i = 1,n
        if (itol .eq. 2 .or. itol .eq. 4) atoli = atol(i)
        ayi = abs(y(i))
...
c-----------------------------------------------------------------------
 ...
 280  if ((tn + h) .ne. tn) go to 290
      ...
      call xerrwv('lsodi--  attention.. t (=r1) and h (=r2) are',
     ...
     1   50, 101, 1, 0, 0, 0, 2, tn, h)
      ...
      call xerrwv('lsodi--  previous message has been given i1 times',
     1   50, 102, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0)
      call xerrwv('     it will not be repeated',
...
c itask = 1.  if tout has been reached, interpolate. -------------------
 310  if ((tn - tout)*h .lt. 0.0d+0) go to 250
      call intdy (tout, 0, rwork(lyh), nyh, y, iflag)
      t = tout
      go to 420
...
c the maximum number of steps was taken before reaching tout. ----------
 500  call xerrwv('lsodi--  at t (=r1), mxstep (=i1) steps   ',
...
c ewt(i) .le. 0.0 for some i (not at start of problem). ----------------
 510  ewti = rwork(lewt+i-1)
      call xerrwv('lsodi--  at t (=r1), ewt(i1) (r2) is .le. 0',
     1   50, 202, 1, 1, i, 0, 2, tn, ewti)
      istate = -6
      go to 590
 520  call xerrwv('lsodi--  at t (=r1),  too much precision required',
...
c kflag = -1.  error test failed repeatedly or with abs(h) = hmin. -----
 530  call xerrwv('lsodi--  at t(=r1) anf for h(=r2), error',
     1   50, 204, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0)
      call xerrwv('      test failed with abs(h) = hmin',
     1   50, 204, 1, 0, 0, 0, 2, tn, h)
      istate = -4
      go to 570
c kflag = -2.  convergence failed repeatedly or with abs(h) = hmin. ----
 540  call xerrwv('lsodi--  at t (=r1) for step h (=r2), le'    ,
...
c ires = 3 returned by res, despite retries by stodi. ------------------
 550  call xerrwv('lsodi--  at t (=r1) repeated error (ires=3) due to ',
     1   50, 206, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0 )
      call xerrwv('routine which evaluates the residue', 30, 206, 1,
     1   0, 0, 0, 1, tn, 0.0d+0 )
      istate = -7
      go to 590
c ainvg failed because a-matrix was singular. --------------------------
 560  ier = -ier
      call xerrwv(
     1 'lsodi-- initialization failed dy/dt: singular matrix',
     1   60, 207, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0 )
      call xerrwv('dgefa or dgbfa return info=(i1)',
     2   50, 207, 1, 1, ier, 0, 0, 0.0d+0, 0.0d+0 )
      istate = -8
      return
c ainvg failed because res set ires to 2 or 3. -------------------------
 565  call xerrwv('lsodi--  initialisation failed dy/dt:  routine',
     1   50, 208, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0 )
      call xerrwv('      of residue evaluation returns:',
     1   50, 208, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0 )
      call xerrwv('       ires = (i1)',
     1   20, 208, 1, 1, ier, 0, 0, 0.0d+0, 0.0d+0 )
      istate = -8
      return
...
c compute residual if relevant. ----------------------------------------
 ...
 585     y(i) = rwork( i+lyh-1 )
     ...
     1    50, 210, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0 )
      call xerrwv('    ires=i1 ' ,
     1     50, 210, 1, 1, ires, 0, 0, 0.0d+0, 0.0d+0 )
      go to 595
...
c-----------------------------------------------------------------------
 ...
 608  call xerrwv('lsodi--  mf (=i1) illegal     ',
     1   30, 8, 1, 1, mf, 0, 0, 0.0d+0, 0.0d+0)
      go to 700
 ...
 611  call xerrwv('lsodi--  maxord (=i1) .lt. 0  ',
     1   30, 11, 1, 1, maxord, 0, 0, 0.0d+0, 0.0d+0)
      go to 700
 ...
 613  call xerrwv('lsodi--  mxhnil (=i1) .lt. 0  ',
     1   30, 13, 1, 1, mxhnil, 0, 0, 0.0d+0, 0.0d+0)
      go to 700
 614  call xerrwv('lsodi--  tout (=r1)  .gt.  t (=r2)      ',
     1   40, 14, 1, 0, 0, 0, 2, tout, t)
      call xerrwv('      h0 (=r1) gives integration direction'  ,
 ...
 616  call xerrwv('lsodi--  hmin (=r1) .lt. 0.0  ',
     1   30, 16, 1, 0, 0, 0, 1, hmin, 0.0d+0)
      go to 700
 617  call xerrwv(
     1 'lsodi-- necessary size for  rwork (i1) larger than i2',
     1   60, 17, 1, 2, lenrw, lrw, 0, 0.0d+0, 0.0d+0)
      go to 700
 618  call xerrwv(
     1 'lsodi-- necessary size for  iwork (i1) larger than i2',
 ...
 621  ewti = rwork(lewt+i-1)
      call xerrwv('lsodi--  ewt(i1) (=r1) is  .le. 0.0         ',
     1   40, 21, 1, 1, i, 0, 1, ewti, 0.0d+0)
      go to 700
 622  call xerrwv(
     1  'lsodi--  tout (=r1) too close to t(=r2) ',
     1   60, 22, 1, 0, 0, 0, 2, tout, t)
      go to 700
 623  call xerrwv(
     1  'lsodi--  itask = i1 and tout (=r1) .gt. tcur - hu (= r2)  ',
     1   60, 23, 1, 1, itask, 0, 2, tout, tp)
      go to 700
 624  call xerrwv(
     1  'lsodi--  itask = 4 or 5 and tcrit (=r1) .gt. tcur (=r2)   ',
     1   60, 24, 1, 0, 0, 0, 2, tcrit, tn)
      go to 700
 625  call xerrwv(
     1  'lsodi--  itask = 4 or 5 and tcrit (=r1)  .gt.  tout (=r2)',
     1   60, 25, 1, 0, 0, 0, 2, tcrit, tout)
      go to 700
 626  call xerrwv('lsodi-- too much accuracy required',
     1   50, 26, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0)
      call xerrwv(
     1 'w.r.t machine precision tolsf (=r1)',
     1   60, 26, 1, 0, 0, 0, 1, tolsf, 0.0d+0)
      rwork(14) = tolsf
      go to 700
 627  call xerrwv('lsodi--  problems due to intdy. itask=i1,tout=r1',
...
c
 700  if (illin .eq. 5) go to 710
      ...
      istate = -3
      return
 710  call xerrwv('lsodi--  incorrect inputs',
     1   50, 302, 1, 0, 0, 0, 0, 0.0d+0, 0.0d+0)
c
 800  call xerrwv('lsodi-- infinite loop',
