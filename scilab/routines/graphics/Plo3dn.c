	      ...
	      if(ppsubwin->axes.reverse[1]==TRUE) /* y is reversed */
		{
		  ...
		  for ( j =(*q)-2 ; j>=0 ; j--)
		    { 
		      ...
		      npolyok += (Gen3DPoints)(flagcolor,polyx,polyy,fill,whiteid,zmin,zmax,
					       xtmp,ytmp,ztmp,i,j,npolyok,p,dc,fg1);
		    }
		  if ( npolyok != 0) {
		  ...
		  for ( j =(*q)-2  ; j >=0 ; j--)
		    {
		      ...
		      npolyok += (Gen3DPoints)(flagcolor,polyx,polyy,fill,whiteid,zmin,zmax,
					       xtmp,ytmp,ztmp,i,(*q)-2-j,npolyok,p,dc,fg1);
		    }
		  if ( npolyok != 0) {
		    if(sciGetIsLine(pobj))
		      C2F(dr)("xliness","str",polyx,polyy,fill,&npolyok,&polysize
		  ...
		  for ( j =0 ; j < (*q)-1 ; j++)
		    {
		      ...
		      /* 				       x,y,ztmp,i,j,npolyok,p,dc,fg1); */
		      npolyok += (Gen3DPoints)(flagcolor,polyx,polyy,fill,whiteid,zmin,zmax,
					       xtmp,y,ztmp,i,j,npolyok,p,dc,fg1);
		    }
		  if ( npolyok != 0) {
		    if(sciGetIsLine(pobj))
		      C2F(dr)("xliness","str",polyx,polyy,fill,&npolyok,&polysize
  ...
  for ( i =0 ; i < *q ; i++)
    {
      ...
      for ( j= 0 ; j < *p ; j++) 
	{
	  xmoy += xtmp[ j +(*p)*i];  ymoy += ytmp[ j +(*p)*i];  zmoy += ztmp[ j +(*p)*i];
	  zdmin =  TRZ(xtmp[ j +(*p)*i]  ,ytmp[ j +(*p)*i]  ,ztmp[ j +(*p)*i]);
  ...
  for ( i = (*q)-1 ; i>= 0 ; i--)
    {
      ...
      for ( j =0 ; j < (*p) ; j++)
	{ 
	  if (trans3d(psubwin ,1, &(polyx[j]),&(polyy[j]),&(xtmp[(*p)*locindex[i]+j]),
		      &(ytmp[(*p)*locindex[i]+j]),&(ztmp[(*p)*locindex[i]+j]))==0)
	    {
	      nok=1;break; 
	    }
	}
      if ( nok == 0) 
	{
	  ...
	  if (( *p >= 2 && ((polyx[1]-polyx[0])*(polyy[2]-polyy[0])-
			    (polyy[1]-polyy[0])*(polyx[2]-polyx[0]))*facteur <  0) && (fg1>=0))
	    { 
	      if (iflag != 0)
	      ...
	      if (fill[0]>0) 
		{
		  ...
		  if(sciGetIsMark(pobj)) 
		    DrawMarks3D(pobj,polysize*npoly,polyx,polyy);
		}
	    }
	  else if ( iflag == 1) 
	    {
	      double zl=0;
	      int k;
	      for ( k= 0 ; k < *p ; k++) zl+= z[(*p)*locindex[i]+k]; /* F.Leray 29.11.04 : DO NOT REPLACE z by ztmp here : zmin & zmax are computed to work with z ! */
