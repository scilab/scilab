...
elseif typeof(instr)=="ifthenelse" then
	...
	for i=1:size(instr._then)
		instr._then(i)=old2newinstr(instr._then(i),oldname,newname)
	end
	for i=1:size(instr._elseifs)
		for k=1:size(instr._elseifs(i)._then)
			instr._elseifs(i)._then(k)=old2newinstr((instr._elseifs(i)._then(k)),oldname,newname)
		end
	end
	for i=1:size(instr._else)
		instr._else(i)=old2newinstr((instr._else(i)),oldname,newname)
	end
...
elseif typeof(instr)=="selectcase" then
	...
	for i=1:size(instr.cases)
		for j=1:size(instr.cases(i)._then)
			instr.cases(i)._then(j)=old2newinstr((instr.cases(i)._then(j)),oldname,newname)
		end
	end
	for i=1:size(instr._else)
		instr._else(i)=old2newinstr(instr._else(i),oldname,newname)
