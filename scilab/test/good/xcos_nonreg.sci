...
function xcos_nonreg(varargin)
    ...
    //      No: list and sort all available tests (*.xcos files)
    ...
    if inputArgs(1) == defaultArgs(1)
        //-- No: list and sort all available tests (*.xcos files)
        xcosFiles   = gsort(basename(listfiles("*.xcos")),"lr","i")
    else
        //-- Yes: check that given list is valid (test existence of diagrams)
        xcosFiles = inputArgs(1)
        idxFilesToDelete = []
        for k = 1:size(xcosFiles,"*")
            if isempty(fileinfo(fullfile(baseDir,cosFiles(k) + ".xcos")))
...
function status = launch_nonreg(baseDir, testName)
    ...
    modelFilename  = fullfile(baseDir, testName + ".xcos")
    ...
    case "4" then
        ...
        if status.ok
            ...
            status.details = [ status.details ; sprintf("     Try running the simulation manually by opening ''%s.xcos'' in Scicos", testName) ]
