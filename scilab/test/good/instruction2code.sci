...
if typeof(I)=="ifthenelse" then
  ...
  [C,indent_space] = format_txt(C,I.then_(1),prettyprint); // Add EOL after then if needed and returns indent_space
  for k=1:size(I.then_)
    C=cat_code(C,indent_space+instruction2code(I.then_(k)))
    if k<size(I.then_) then // Add EOL between then statements if needed
      C = format_txt(C,I.then_(k),prettyprint,I.then_(k+1));
    end
  end
  C = format_txt(C,I.then_($),prettyprint); // Add EOL after last then statement if needed

  // ELSEIF
  if size(I.else_ifs)<>0 then
  for k=1:size(I.else_ifs)
  C=cat_code(C,"elseif "+expression2code(I.else_ifs(k).expression)+" then")
[C,indent_space] = format_txt(C,I.else_ifs(k).then_(1),prettyprint); // Add EOL after then if needed and returns indent_space
  for l=1:size(I.else_ifs(k).then_)
C=cat_code(C,indent_space+instruction2code(I.else_ifs(k).then_(l)))
  if l<size(I.else_ifs(k).then_) then // Add EOL between then statements
  C = format_txt(C,I.else_ifs(k).then_(l),prettyprint,I.else_ifs(k).then_(l+1));
end
end
C = format_txt(C,I.else_ifs(k).then_($),prettyprint); // Add EOL after last then statement if needed
end
end

// ELSE
if size(I.else_)<>0 then
C=cat_code(C,"else")
[C,indent_space] = format_txt(C,I.else_(1),prettyprint); // Add EOL after else if needed and returns indent_space
  for k=1:size(I.else_)
C=cat_code(C,indent_space+instruction2code(I.else_(k)))
if k<size(I.else_) then // Add EOL between else statements if needed
  C = format_txt(C,I.else_(k),prettyprint,I.else_(k+1));
end
end
C = format_txt(C,I.else_($),prettyprint); // Add EOL after last else statement if needed
end
C=cat_code(C,"end")
C($)=C($)+";"
return
...
if typeof(I)=="selectcase" then
  ...
  if size(I.cases)<>0 then
    for k=1:size(I.cases)
      ...
      [C,indent_space] = format_txt(C,I.cases(k).then_(1),prettyprint); // Add EOL after then if needed and returns indent_space
      if indent_space=="  " then // indent_space is modified because indentation differs from others control instructions
	indent_space="    "
      end
      for l=1:size(I.cases(k).then_)
	C=cat_code(C,indent_space+instruction2code(I.cases(k).then_(l)))
	if l<size(I.cases(k).then_) then // Add EOL between then statements if needed
	  C = format_txt(C,I.cases(k).then_(l),prettyprint,I.cases(k).then_(l+1));
	end
      end
      C = format_txt(C,I.cases(k).then_($),prettyprint); // Add EOL after last then statement if needed
    end
  end
  // ELSE
  if size(I.else_)<>0 then
