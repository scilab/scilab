...
case 'set' then
  ...
  while %t do
    [ok,junction_name,funtyp,i,it,o,ot,ci,co,xx,z,rpar,ipar,nmode,nzcr,auto0,depu,dept,lab]=..
    ...
    i=int(i);
    it=int(it(:));
    if max(it)>8 then message('maximum input ports type must be <= 8'), end;
    if size(i,1)<>size(it,1) then 
      message('rows of input ports sizes and size of type must be equal')
      ok=%f;
    end
    o=int(o);
    ot=int(ot(:));
    if max(ot)>8 then message('maximum output ports type must be <= 8'), end;
    if size(o,1)<>size(ot,1) then 
      message('rows of output ports size and size of type must be equal')
      ok=%f;
    end
    ci=int(ci(:));
    co=int(co(:));
    ...
    dep_ut=[depu dept];
    if ok then
      [model,graphics,ok]=check_io(model,graphics,i(:,1),o(:,1),ci,co)
    end
    if ok then
      // AVERIFIER
