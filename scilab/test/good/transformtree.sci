...
if typeof(instr)=="ifthenelse" then
  for i=1:size(instr._then)
    instr._then(i)=transformtree((instr._then(i)))
  end
  for i=1:size(instr.elseifs)
  for k=1:size(instr.elseifs(i)._then)
  instr.elseifs(i)._then(k)=transformtree((instr.elseifs(i)._then(k)))
end
end
for i=1:size(instr._else)
instr._else(i)=transformtree((instr._else(i)))
end
elseif typeof(instr)=="selectcase" then
  for i=1:size(instr.cases)
    for j=1:size(instr.cases(i)._then)
      instr.cases(i)._then(j)=transformtree((instr.cases(i)._then(j)))
    end
  end
  for i=1:size(instr._else)
  instr._else(i)=transformtree(instr._else(i))
