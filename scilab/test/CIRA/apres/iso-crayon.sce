...
for fl=1:1:4;
    ...
    a=gca();
    a.auto_clear="on";
    plot(w,TIT1,"k",w,TIT3,"r",w,TIT5,"b",w,TIT8,"m",w,FIT1/10,"k",w,FIT2/10,"b",w,FIT3/1000,"r");
    xgrid();
    ...
    for j =1:1:90 // for (1)
        ...
        for ii=2+r:1:tf;
            ...
            if ( ii > tinit ) & ( ii < tfin ) then
                VMM=VMM+sm(ii)*sm(ii); VMP=VMP+sm(ii)*sp(ii);
            end
        end
        Vmm(j)=VMM;
        Vmp(j)=VMP;
    ...
    for j=1:1:90
        ...
        if (det > 0 );
            k1(j)=(Vmp(j)-sqrt(det))/Vmm(j);
            k2(j)=(Vmp(j)+sqrt(det))/Vmm(j);
        else
            k1(j)=Vmp(j)/Vmm(j);
            k2(j)=k1(j);
        end
    ...
    for u=1:1:100
        tt(u)=(u-1)*60/u;
        kk(u)=tt(u)*A;
    end// le rapport K/tau doit être constant,
    ...
    //tracé modelopt;
    for ii=2+r:1:tf
        sm(ii)=sm(ii-1)*am+bm*e(ii-1-r)*kopt;
    end;
