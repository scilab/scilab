...
K2= 2;
tau2=30 ;
a2=exp(-tech/tau2);b2=1-a2;//disturbance
kf=1;
tauf =5;
af=exp(-tech/tauf);
bf=1-af;
...
for ii =2+rt:1:tf
    //disturbance
    if ( ii > 150 ) then Pert(ii)=30;end
    if ( ii > 500 ) then Pert(ii)=30+20*sin(2*(%pi)*ii/200);end
...
if ( fl == 1 ) then
    title( " COMMANDE AVEC PRISE EN TENDANCE  ","fontsize",3);
end

if ( fl == 0 ) then
    title( " COMMANDE SANS PRISE EN TENDANCE  ");
end
//----------------------------------------------------------------------------
