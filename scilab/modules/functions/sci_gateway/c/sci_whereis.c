...
/*--------------------------------------------------------------------------*/
int sci_whereis(char *fname, int* _piKey)
{
	SciErr sciErr;
	...
	else if (iType1 == sci_strings)
	{
		...
		sciErr = getMatrixOfString(_piKey, piAddressVarOne, &m, &n, &lenStVarOne, &pStVarOne);
		if(sciErr.iErr)
		{
			printError(&sciErr, 0);
			return 0;
		...
		else
		{
			...
			sciErr = createMatrixOfString(_piKey, Rhs + 1, m_out, n_out, librariesResult);
			if(sciErr.iErr)
			{
				printError(&sciErr, 0);
				return 0;
