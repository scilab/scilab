...
class SLintContext
{
    ...
    const SLint & project;
    std::unordered_map<std::string, const ast::FunctionDec *> publicFunctions;
    SciFilePtr currentFile;
    const ast::Exp * LHSExp;
    std::stack<const ast::FunctionDec *> funStack;
    std::stack<const ast::Exp *> loopStack;
    std::unordered_set<std::string> funIn;
    std::unordered_set<std::string> funOut;
...
public:
    ...
    const std::unordered_set<std::string> & getFunIn() const;
    const std::unordered_set<std::string> & getFunOut() const;
    bool isFunIn(const symbol::Symbol & sym) const;
    bool isFunOut(const symbol::Symbol & sym) const;
    bool isFunIn(const std::string & name) const;
    bool isFunOut(const std::string & name) const;
    ...
    std::string getRHSCallName() const;
    bool isPrivateFunction(const symbol::Symbol & sym) const;
    const ast::FunctionDec * getPrivateFunction(const std::string & name) const;
    bool isExternPrivateFunction(const symbol::Symbol & sym, std::string & name) const;
    void addPublicFunction(const ast::FunctionDec * fundec);
    const ast::FunctionDec * getPublicFunction(const std::string & name) const;
