...
public:
    ...
    DecimalChecker(const std::string & checkerId, const std::string& _character) : SLintChecker(checkerId), character(_character), checkDot(false) { }
    DecimalChecker(const std::string & checkerId, const bool _checkDot) : SLintChecker(checkerId), character('\0'), checkDot(_checkDot) { }
    DecimalChecker(const std::string & checkerId, const std::string& _character, const bool _checkDot) : SLintChecker(checkerId), character(_character), checkDot(_checkDot) { }
