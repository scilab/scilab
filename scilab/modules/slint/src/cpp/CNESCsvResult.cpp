...
CNESCsvResult::CNESCsvResult(const ToolConfiguration & tc, types::String * conf, const std::string & id, const std::string & _path) : tct(tc.getToolConfiguration()), current(nullptr), path(_path)
{
...
void CNESCsvResult::handleMessage(SLintContext & context, const Location & loc, const SLintChecker & checker, const unsigned sub, const std::string & msg)
{
...
void CNESCsvResult::printRes()
{
    if (current.get())
    {
        for (const auto & r : res)
        {
            ...
            {
                ruleName = getStr(i->second.getStandardRuleId());
            }
