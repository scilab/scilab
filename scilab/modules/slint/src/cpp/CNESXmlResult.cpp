...
CNESXmlResult::CNESXmlResult(const ToolConfiguration & tc, types::String * conf, const std::string & id, const std::string & _path) : tct(tc.getToolConfiguration()), current(nullptr), path(_path)
{
...
void CNESXmlResult::handleMessage(SLintContext & context, const Location & loc, const SLintChecker & checker, const unsigned sub, const std::string & msg)
{
...
void CNESXmlResult::printRes()
{
    if (current.get())
    {
        for (const auto & r : res)
        {
            ...
            {
                ruleName = SLintXmlResult::getStr(i->second.getStandardRuleId());
            }
