...
{
    int m1 = 0, n1 = 0, l1 = 0, m2 = 0, n2 = 0, it2 = 0, l2 = 0, l2r = 0;
    int l2i = 0, mr = 0, nr = 0, itr = 0, lr = 0, li = 0, lwi = 0, lwr = 0;
    ...
    { 
        ...
        if (itr == 0)
        {
            C2F(dbesjv) (stk(l2),&nx,stk(l1),&na,&kode,stk(lr),stk(llwr),&ierr);
        }
        else
        {
            C2F(zbesjv) (stk(l2),stk(l2i),&nx,stk(l1),&na,&kode,stk(lr),stk(li),stk(llwr),stk(llwi),&ierr);
        }
    ...
    { 
        /* besselj(row,col) or besselj(col,row) */
        int lun = 1, nw = 0;
        mr = m2 * n2;
        nr = m1 * n1;
        CreateCVar(lpos+1,MATRIX_OF_DOUBLE_DATATYPE,&itr,&mr,&nr,&lr,&li);
