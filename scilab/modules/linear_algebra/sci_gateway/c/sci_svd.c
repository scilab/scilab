...
*/
static int copyStrArrayArg(int* _piKey, int* varAdr, int* pRows, int* pCols, char*** strArrayArg );
static void freeStrArray(char** strArray, int size);

extern int C2F(vfinite)(int *n, double *v);

static int isEconomyMode(int* _piKey, int*const arg2);

static int handleEmptyMatrix(int* _piKey);

static int allocU_S_V(int* _piKey, int rows, int cols, int economyRows, int economyCols, int isComplex, double** ptrsU, double** pS, double** ptrsV);


int sci_svd(char *fname, int* _piKey)
{
  ...
  if ( Rhs >=1 )
	{
		...
		else
		{
			...
			if(Rhs == 2)
			{
				getVarAddressFromPosition(_piKey, 2, &adr2);
				economy= isEconomyMode(_piKey, adr2);
			}
				...
				else
				{
					...
					else
					{
						if(Lhs >= 3)
						{
							...
							allocU_S_V(_piKey, iRows, iCols, economyRows, economyCols, complexArg, ptrsU, &pS, ptrsV);

							if(Lhs == 4)
							{
								getVarType(_piKey, adr2, &type);
								if( (Rhs == 2 ) && (type == sci_matrix)) /*  getVarAddressFromPosition(2, &adr2) was already called */
								{
									...
									double* tmpData;
									getMatrixOfDouble(_piKey, adr2, &dummy, &dummy, &tmpData);
									tol= *tmpData;
								}
								allocMatrixOfDouble(_piKey, Rhs+4, 1, 1, &pRk);
						...
						else
						{
							...
							{
								allocMatrixOfDouble(_piKey, Rhs+1, Min(iRows, iCols), 1, &pSV);
							}
						}
...
/*--------------------------------------------------------------------------*/
int copyStrArrayArg(int* _piKey, int* varAdr, int* pRows, int* pCols, char*** strArrayArg )
{
	int * lengths=NULL;
	int ret=0;

	getMatrixOfString(_piKey, varAdr, pRows, pCols, NULL, NULL);
	{
		int i;
		int size= *pRows * *pCols;
		lengths= MALLOC(size * sizeof(int));
		getMatrixOfString(_piKey, varAdr, pRows, pCols, lengths, NULL);
	...
	getMatrixOfString(_piKey, varAdr, pRows, pCols, lengths, *strArrayArg);
	FREE(lengths);
	return ret;
}
...
int isEconomyMode(int* _piKey, int*const adr2)
{
	int type;
  int economy= 0;
  if(adr2)
	{
		...
		switch(type)
		{
			case sci_strings:
			{
				char** strArray;
				int rows, cols;
				copyStrArrayArg(_piKey, adr2, &rows, &cols, &strArray);
...
int handleEmptyMatrix(int* _piKey)
{
  ...
  allocMatrixOfDouble(_piKey, 3, 0, 0, &data);
  LhsVar(1)= 3;
  if(Lhs >= 2)
	{
		allocMatrixOfDouble(_piKey, 4, 0, 0, &data);
		LhsVar(2)= 4;
	}

  if(Lhs >=3)
	{
		allocMatrixOfDouble(_piKey, 5, 0, 0, &data);
		LhsVar(3)= 5;
	}

  if(Lhs == 4)
	{
		allocMatrixOfDouble(_piKey, 6, 1, 1, &data);
		*data= 0.;
		LhsVar(4)= 6;
	}
  return 0;
...
static int allocU_S_V(int* _piKey, int rows, int cols, int economyRows, int economyCols, int isComplex, double* ptrsU[], double** ptrS, double* ptrsV[])
{
  if(isComplex)
	{
		allocComplexMatrixOfDouble(_piKey, Rhs+1, rows, economyRows, ptrsU+1, ptrsU+2);
		ptrsU[0]= (double*)MALLOC(rows * economyRows*(isComplex ? sizeof(doublecomplex): sizeof(double)));
		allocMatrixOfDouble(_piKey, Rhs+2, economyRows, economyCols, ptrS);
		allocComplexMatrixOfDouble(_piKey, Rhs+3, cols, economyCols, ptrsV+1, ptrsV+2);
		ptrsV[0]= (double*)MALLOC(cols * economyCols*(isComplex ? sizeof(doublecomplex): sizeof(double)));
	}
  else
	{
		ptrsU[1]= ptrsU[2]= NULL;
		allocMatrixOfDouble(_piKey, Rhs+1, rows, economyRows, ptrsU+0);
		allocMatrixOfDouble(_piKey, Rhs+2, economyRows, economyCols, ptrS);
		ptrsV[1]= ptrsV[2]= (double*)NULL;
		allocMatrixOfDouble(_piKey, Rhs+3, cols, economyCols , ptrsV+0);
	}
  return 0;
}
