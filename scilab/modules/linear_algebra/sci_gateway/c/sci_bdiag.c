...
int sci_bdiagr(char *fname, int* _piKey)
{
  int ret= 0;
  ...
  for(i=0; i != Rhs; ++i)
    {
      getVarAddressFromPosition(_piKey, i+1, &arg[i]);
      getVarType(_piKey, arg[i], &type);
      if(type != sci_matrix)
	{
	  Scierror(201,_("%s: Wrong type for argument %d: Real or complex matrix expected.\n"),fname, i +1);
	  ret= 201;
	}
      if(isVarComplex(_piKey, arg[i] ))
	{
  ...
  if(!ret)
	...
	{
	  double* dummy;
	  allocMatrixOfDouble(_piKey, Rhs+1, 0, 0, &dummy);
	  allocMatrixOfDouble(_piKey, Rhs+2, 0, 0, &dummy);
      ...
      else
	{
	  ...
	  if (  ! (C2F(vfinite)(&totalSize, pDataReal[0]) &&(!isVarComplex(_piKey, arg[0]) || (C2F(vfinite)(&totalSize, pDataImg[0])))))
	    {
	      ...
	      if(le && lib && lw && !stackAllocError)
		{
		  ...
		  if (fail)
		      ...
		      if (Lhs == 3)
			{
			  ...
			  allocMatrixOfDouble(_piKey, Rhs + 2, nbloc, 1, &lbs);
