...
int C2F(intbdiagr)(char *fname, int* _piKey)
{
	...
	for(i=0; i != Rhs; ++i)
	{
		getVarAddressFromPosition(i+1, &arg[i], _piKey);
	...
	if(!ret)
	{
		...
		{
			double* dummy;
			allocMatrixOfDouble(Rhs+1, 0, 0, &dummy, _piKey);
			allocMatrixOfDouble(Rhs+2, 0, 0, &dummy, _piKey);
					...
					if(le && lib && lw && !stackAllocError)
					{
						...
						else
						{
							...
							if (Lhs == 3) 
							{
								...
								allocMatrixOfDouble(Rhs + 2, nbloc, 1, &lbs, _piKey);
