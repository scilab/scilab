...
int C2F(intlsq)(char *fname,unsigned long fname_len)
{
	...
	if (Rhs>=1)
	{
...

		{
			...
			complexArgs= isVarComplex(pvApiCtx, arg[0]) || isVarComplex(pvApiCtx, arg[1]);
			for(i=0; i != 2 ; ++i)
			{
				int iPlus1= i+1;
...
#else
					getMatrixOfDouble(pvApiCtx, arg[i], &iRows[i], &iCols[i], &pData[i]);
#endif
				}
			...
			if(Rhs == 3)
			{
				getVarAddressFromPosition(pvApiCtx, 3, &arg[2]);
				getVarType(pvApiCtx, arg[2], &type);
				if(type != sci_matrix)
...
#else
					getMatrixOfDouble(pvApiCtx, arg[2],&unused, &unused, &pTol);
#endif
				}
			}
			...
			if(!ret)
			{
				...
				if(complexArgs)
				{
					...
					allocComplexMatrixOfDouble(pvApiCtx, Rhs+1, iCols[0], iCols[1] , &pResultReal, &pResultImg);
				}
				else
				{
					allocMatrixOfDouble(pvApiCtx, Rhs+1, iCols[0], iCols[1], &pResult);
				}
				...
				if(ret==0)
				{
					...
					if(Lhs == 2)
					{
						...
						if(emptyMatrix)
						{
							allocMatrixOfDouble(pvApiCtx, Rhs+2, 0, 0, &pRank);
						}
						else
						{
							allocMatrixOfDouble(pvApiCtx, Rhs+2, 1, 1, &pRank);
							*pRank= (double) rank;
						}
