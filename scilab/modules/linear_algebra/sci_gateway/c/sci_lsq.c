...
int C2F(intlsq)(char *fname,unsigned long fname_len)
{
    ...
    }
  {
    ...
    complexArgs= iIsComplex(1) || iIsComplex(2);
    for(i=0; i != 2 ; ++i)
      {
	int iPlus1= i+1;
	if(complexArgs)
	...
	ret= complexArgs 
	  ...
	  ,iAllocComplexMatrixOfDouble(Rhs+1, iCols[0], iCols[1] , &pResultReal, &pResultImg)
	  : iAllocMatrixOfDouble(Rhs+1, iCols[0], iCols[1], &pResult) ;
	if(ret==0)
	  {
		...
		if(emptyMatrix)
		  {
		    iAllocMatrixOfDouble(Rhs+2, 0, 0, &pRank);
		  }
		else
		  {
		    iAllocMatrixOfDouble(Rhs+2, 1, 1, &pRank);
		    *pRank= (double) rank;
		  }
		LhsVar(2)= Rhs + 2;
	      }
