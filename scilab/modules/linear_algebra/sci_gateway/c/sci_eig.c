...
int sci_eig(char *fname, int* _piKey)
{
	int* arg[2]= {NULL, NULL};
	int type;
	int ret=0;
		...
		else
		{
			switch (Rhs)
			{
				case 1:
				{
					...
					int complexArg= isVarComplex(_piKey, arg[0]);
					CheckLhs(1,2);
					if(complexArg)
					{
					...
					{
						getMatrixOfDouble(_piKey, arg[0], &iRows, &iCols, &pDataReal);
					}
					if( iCols == -1)
					{
					...
					else /* not eye() matrix */
					{
						if (iCols==0)
						{
							...
							else /* Lhs == 2 */
							{
								...
								if(complexArg)
								{
									allocComplexMatrixOfDouble(_piKey, 2, iCols, iCols, &dummy, &dummy);
								}
								else
								{
									allocMatrixOfDouble(_piKey, 2, iCols, iCols, &dummy);
								}
						...
						{/* now the interesting case, can be either complex or real and symmetric or not */
							...
							{
								allocComplexMatrixOfDouble(_piKey, 2, iCols, eigenValuesCols, &pEigenValuesReal, &pEigenValuesImg);
								if(Lhs==2)
								{
									allocComplexMatrixOfDouble(_piKey, 3, iCols, iCols, &pEigenVectorsReal, &pEigenVectorsImg);
								}
							}
				...
				case 2:
				{
...

					{
						...
						*/
						int complexArgs= isVarComplex(_piKey, arg[0]) || isVarComplex(_piKey, arg[1]);
						...
						else
						{
							for(i=0; i!=2; ++i)
							{
								getMatrixOfDouble(_piKey, arg[i], &iRows[i], &iCols[i], &pDataReal[i]);
							}
						}
						...
						if( iCols[0] == 0)
						{
							switch (Lhs)
							{
								case 4:
								{
									...
									if(complexArgs)
									{
										allocComplexMatrixOfDouble(_piKey, 4, 0, 0, &dummy, &dummy);
									}
									else
									{
										allocMatrixOfDouble(_piKey, 4, 0, 0, &dummy) ;
									}
								...
								case 3:
								{
									...
									if(complexArgs)
									{
										allocComplexMatrixOfDouble(_piKey, 3, 0, 0, &dummy, &dummy);
									}
									else
									{
										allocMatrixOfDouble(_piKey, 3, 0, 0, &dummy) ;
									}
							...
							if(!inf)
							{
								...
								*/
								...
								{/* for Complex case, it could be possible to reuse var from Rhs1 & Rhs2 for Lhs 3 & Lhs4 */
									case 4:
										allocComplexMatrixOfDouble(_piKey, 6, iCols[0], iCols[0], &pLReal, &pLImg);  /* nobreak */
									case 3:
										allocComplexMatrixOfDouble(_piKey, 5, iCols[0], iCols[0], &pRReal, &pRImg); /* nobreak */
									case 2:
									{
										if(complexArgs && !complexArgsWithZeroImg)
										{
											allocComplexMatrixOfDouble(_piKey, 4, iCols[0], 1, &pBetaReal, &pBetaImg);
											pBeta= (doublecomplex*)MALLOC(iCols[0] * sizeof(doublecomplex));
										}
										else
										{
											allocMatrixOfDouble(_piKey, 4, iCols[0], 1, &pBetaReal);
										}
									} /* nobreak */

									default : /*case 1 as we have done CheckLhs(1,4) */
									{ /* note that real matrix use alpha in 'c' format while complex matrix use it in 'z' format, cf dggev and zggev */
										allocComplexMatrixOfDouble(_piKey, 3, iCols[0], 1, &pAlphaReal, &pAlphaImg);
										if(!complexArgs || complexArgsWithZeroImg)
										{
											pAlpha = NULL;
										}
