		...
		else
		{
			switch (Rhs)
			{
				case 1:
				{
					...
					int complexArg= isVarComplex(pvApiCtx, arg[0]);
					CheckLhs(1,2);
					if(complexArg)
					{
#ifdef STACK3
...
#else
						getMatrixOfDouble(pvApiCtx, arg[0], &iRows, &iCols, &pDataReal);
#endif
					}
					...
					else /* not eye() matrix */
					{
						if (iCols==0)
						{
							...
							else /* Lhs == 2 */
							{
								...
								if(complexArg)
								{
									allocComplexMatrixOfDouble(pvApiCtx, 2, iCols, iCols, &dummy, &dummy);
								}
								else
								{
									allocMatrixOfDouble(pvApiCtx, 2, iCols, iCols, &dummy);
								}
						...
						{/* now the interesting case, can be either complex or real and symmetric or not */
							...
							{
								allocMatrixOfDouble(pvApiCtx, 2, iCols, eigenValuesCols, &pEigenValuesReal);
								/* if matrix is symmetric, the eigenvectors can reuse Rhs because the matrix is of the same type & dimensions */
							}
							else
							{
								allocComplexMatrixOfDouble(pvApiCtx, 2, iCols, eigenValuesCols, &pEigenValuesReal, &pEigenValuesImg);
								if(Lhs==2)
								{
									allocComplexMatrixOfDouble(pvApiCtx, 3, iCols, iCols, &pEigenVectorsReal, &pEigenVectorsImg);
								}
							}
				...
				case 2:
				{
...

					{
						...
						*/
						int complexArgs= isVarComplex(pvApiCtx, arg[0]) || isVarComplex(pvApiCtx, arg[1]);
						...
						if( iCols[0] == 0)
						{
							switch (Lhs)
							{
								case 4:
								{
									...
									if(complexArgs)
									{
										allocComplexMatrixOfDouble(pvApiCtx, 4, 0, 0, &dummy, &dummy);
									}
									else
									{
										allocMatrixOfDouble(pvApiCtx, 4, 0, 0, &dummy) ;
									}
								...
								case 3:
								{
									...
									if(complexArgs)
									{
										allocComplexMatrixOfDouble(pvApiCtx, 3, 0, 0, &dummy, &dummy);
									}
									else
									{
										allocMatrixOfDouble(pvApiCtx, 3, 0, 0, &dummy) ;
									}
						...
						else
						{
							...
							if(!inf)
							{
								...
								*/
								...
								switch(Lhs)
								{/* for Complex case, it could be possible to reuse var from Rhs1 & Rhs2 for Lhs 3 & Lhs4 */
									case 4:
										allocComplexMatrixOfDouble(pvApiCtx, 6, iCols[0], iCols[0], &pLReal, &pLImg);  /* nobreak */
									case 3:
										allocComplexMatrixOfDouble(pvApiCtx, 5, iCols[0], iCols[0], &pRReal, &pRImg); /* nobreak */
									case 2:
									{
										if(complexArgs && !complexArgsWithZeroImg)
										{
											allocComplexMatrixOfDouble(pvApiCtx, 4, iCols[0], 1, &pBetaReal, &pBetaImg);
											pBeta= (doublecomplex*)MALLOC(iCols[0] * sizeof(doublecomplex));
										}
										else
										{
											allocMatrixOfDouble(pvApiCtx, 4, iCols[0], 1, &pBetaReal);
										}
									} /* nobreak */

									default : /*case 1 as we have done CheckLhs(1,4) */
									{ /* note that real matrix use alpha in 'c' format while complex matrix use it in 'z' format, cf dggev and zggev */
										allocComplexMatrixOfDouble(pvApiCtx, 3, iCols[0], 1, &pAlphaReal, &pAlphaImg);
										if(!complexArgs || complexArgsWithZeroImg)
										{
											pAlpha = NULL;
										}
										...
										{
											pAlpha = (doublecomplex*) MALLOC(iCols[0] * sizeof(doublecomplex));
										}
									}
									/* nobreak */
								}
