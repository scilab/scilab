...
int C2F(inteig)(char *fname, int* _piKey)
{
	...
	if(Rhs >= 1)
	{
		...
		case 1:{
			...
			else /* not eye() matrix */
			{
				if (iCols==0)
				{
					...
					else /* Lhs == 2 */
					{
						...
						int unused = complexArg 
							? allocComplexMatrixOfDouble(2, iCols, iCols, &dummy, &dummy, _piKey)
							: allocMatrixOfDouble(2, iCols, iCols, &dummy, _piKey);
				...
				{/* now the interesting case, can be either complex or real and symmetric or not */
					...
					{
						allocMatrixOfDouble(2, iCols, eigenValuesCols, &pEigenValuesReal, _piKey);
						/* if matrix is symmetric, the eigenvectors can reuse Rhs because the matrix is of the same type & dimensions */
					}
					else
					{
						allocComplexMatrixOfDouble(2, iCols, eigenValuesCols, &pEigenValuesReal, &pEigenValuesImg, _piKey);
						if(Lhs==2)
						{
							allocComplexMatrixOfDouble(3, iCols, iCols, &pEigenVectorsReal, &pEigenVectorsImg, _piKey);
						}
					}
					if( complexArg )
					{
		...
		case 4:{
			...
			if(complexArgs)
			{
				allocComplexMatrixOfDouble(4, 0, 0, &dummy, &dummy, _piKey);
			}
			else
			{
				allocMatrixOfDouble(4, 0, 0, &dummy, _piKey);
			}
			LhsVar(4)= 4; /* no break */
					 }
		case 3:{
			...
			if(complexArgs)
			{
				allocComplexMatrixOfDouble(3, 0, 0, &dummy, &dummy, _piKey);
			}
			else
			{
				allocMatrixOfDouble(3, 0, 0, &dummy, _piKey);
			}
			LhsVar(3)= 3;
					 }/* no break */
		case 2:
		...
		case 4:
			allocComplexMatrixOfDouble(6, iCols[0], iCols[0], &pLReal, &pLImg, _piKey);  /* nobreak */
		case 3:
			allocComplexMatrixOfDouble(5, iCols[0], iCols[0], &pRReal, &pRImg, _piKey); /* nobreak */
		case 2:{
			if(complexArgs && !complexArgsWithZeroImg)
			{
				pBeta= allocComplexMatrixOfDouble(4, iCols[0], 1, &pBetaReal, &pBetaImg, _piKey)
			...
			{
				allocMatrixOfDouble(4, iCols[0], 1, &pBetaReal, _piKey);
			}
					 } /* nobreak */
		default : /*case 1 as we have done CheckLhs(1,4) */
			{ /* note that real matrix use alpha in 'c' format while complex matrix use it in 'z' format, cf dggev and zggev */
				pAlpha = ( allocComplexMatrixOfDouble(3, iCols[0], 1, &pAlphaReal, &pAlphaImg, _piKey) 
