...
static schur_select_type getDynamicFunctionByName(char* name)
{
    int err = 0;
...
static schur_select_type createSelectWrapper(int isCplx, int twoMat)
{
    if (isCplx)
    {
        if (twoMat)
        {
            checkZggesSciSelectFunction();
        }
        else
        {
            checkZgeesSciSelectFunction();
        }
    }
    else
    {
        if (twoMat)
        {
            checkDggesSciSelectFunction();
        }
        else
        {
            checkDgeesSciSelectFunction();
        }
    }
}

static int isNameOneOf(char* name, char* n1, char* n2, char* n3)
{
    return !(strcmp(name, n1) && strcmp(name, n2) && strcmp(name, n3));
}

static schur_select_type selectFunctionByName(char* name
        , char* contName, schur_select_type contFunction
        , char* discName, schur_select_type discFunction)
{
...
static schur_select_type getFunctionByName(int isCplx, int twoMat, char* name)
{
...
static int createLhsOpts(int isCplx, int cols, data_matrix_type* lhsOpt1, int* vsIndexPtr, data_matrix_type* lhsOpt2, int* vsRIndexPtr)
{
    int ret = 0;
    int i;
    int nbVs = lhsOpt2 ?  2 : (lhsOpt1 ? 1 : 0);
    for (i = 0; i != nbVs; ++i)
    {
...
#define HANDLE_LHS
    ...
    if (!externalFunction)
    {
        ...
        else
        {
            ...
            {
                case 4:
                    LhsVar(4) = Rhs + 2; /* no break */
                case 3:
                    LhsVar(3) = Rhs + 1; /* no break */
                case 2:
                    LhsVar(2) = twoMat ? 2 : Rhs + 1 ; /* no break */
                case 1:
                    LhsVar(1) = 1;
            }
        }
        ...
        if (twoMat)
        {
            ...
            else
            {
                switch (Lhs)
                {
                    default:
                    case 2:
                        LhsVar(Lhs - 1) = vsR; /* nobreak */
                    case 1:
                        LhsVar(Lhs) = dim; /* no break */
                }
            }
        }
        else
...
int C2F(intschur)(char *fname, unsigned long fname_len)
{
    ...
    CheckLhs(1, 5); /* doc says min Lhs == 2, but unit tests disagree :( */
    {
        ...
        char* strArg = optionPlace ? getStringArg(optionPlace) : NULL;
...
static int dggesSciSelectFunction (double const* alphaReal, double const* alphaImg, double const* beta)
{
    ...
    *stk(dataImg) = *alphaImg;
    ...
    {
        int secondArg = beginSelectArg + 1;
