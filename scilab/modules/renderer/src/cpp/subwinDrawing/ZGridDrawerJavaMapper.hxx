/*------------------------------------------------------------------------*/
/* file: ZGridDrawerJavaMapper.hxx                                        */
/* Copyright INRIA 2007                                                   */
/* Authors : Jean-Baptiste Silvy                                          */
/* desc : Class containing java methods needed by ZGridDrawerJoGL         */
/*------------------------------------------------------------------------*/

#ifndef _Z_GRID_DRAWER_JAVA_MAPPER_HXX_
#define _Z_GRID_DRAWER_JAVA_MAPPER_HXX_

#include "GridDrawerJavaMapper.hxx"
#include "ZGridDrawerGL.hxx"

namespace sciGraphics
{

/**
* Contains mapping of Java method used by XGridDrawerJoGL 
*/
class ZGridDrawerJavaMapper : public virtual GridDrawerJavaMapper
{
public:

  ZGridDrawerJavaMapper(void);

  virtual ~ZGridDrawerJavaMapper(void);

  /*----------------------------------------------------------------------*/
  // Inherited from DrawableObjectJavaMapper
  virtual void display(void);

  virtual void initializeDrawing(int figureIndex);
  virtual void endDrawing(void);

  virtual void show(int figureIndex);

  virtual void destroy(int figureIndex);

  virtual void setFigureIndex(int figureIndex);
  /*----------------------------------------------------------------------*/
  // specific for TicksDrawer
  virtual void drawGrid(const double gridPositions[], int nbPositions);

  virtual void setGridParameters(int gridColor, float gridThickness);

  virtual void setAxesBounds(double xMin, double xMax,
                             double yMin, double yMax,
                             double zMin, double zMax);
  /*----------------------------------------------------------------------*/

protected:

  org_scilab_modules_renderer_subwinDrawing::ZGridDrawerGL * m_pJavaObject;

};

}

#endif /* _Z_GRID_DRAWER_JAVA_MAPPER_HXX_ */
