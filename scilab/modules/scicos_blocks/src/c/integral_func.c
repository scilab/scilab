...
void  integral_func(scicos_block *block,int flag)
{
  ...
  if (flag == 0) //++ Derivative computation
  {
    if(block->ng > 0)
    {
  ...
  else if (flag == 1 || flag == 6) //++ Outputs update or Reinitialization
  {
    for(i = 0; i < block->nx ; ++i)
    {
  ...
  else if (flag == 2 && block->nevprt == 1) //++ States update
  {
    for(i = 0; i < block->nx ; ++i)
    {
  ...
  else if (flag == 9) //++ Mode and zero-crossing
  {
    for(i = 0; i < block->nx ; ++i)
    {
      if (block->mode[i]==3)
