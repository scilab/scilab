...
function [x,y,typ]=INTRP2BLK_f(job,arg1,arg2)
    ...
    case "set" then
        ...
        model=arg1.model;
        while %t do
            [ok,a,b,c,exprs]=scicos_getvalue(..
            _("Set Interpolation block parameters"),..
            _(["X coord.";"Y coord.";"Z values"]), ..
            list("vec",-1,"vec",-1,"mat",[-1,-1]),exprs)
            if ~ok then
                break,
            end
            if size(a,"*") <> size(c,"c") | size(b,"*") <> size(c,"r") then
                message(_("X, Y, Z: incompatible sizes"))
            elseif min(a(2:$)-a(1:$-1)) <=0 | min(b(2:$)-b(1:$-1)) <=0 then
                message(_("X and Y must be strictly increasing"))
            else
                if ok then
