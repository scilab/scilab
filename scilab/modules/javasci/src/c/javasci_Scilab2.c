...
JNIEXPORT void JNICALL Java_javasci_Scilab_sendDoubleMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	strErr = createNamedMatrixOfDouble((char*)cname, nbRow, nbCol, matrix);
	if(strErr.iErr)
	{
...
JNIEXPORT void JNICALL Java_javasci_Scilab_receiveDoubleMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	strErr = readNamedMatrixOfDouble((char*)cname, &rows, &cols, NULL);
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
	...
	strErr = readNamedMatrixOfDouble((char*)cname, &rows, &cols, matrix);
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
	}
...
JNIEXPORT void JNICALL Java_javasci_Scilab_sendComplexMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	strErr = createNamedComplexMatrixOfDouble((char*)cname, nbRow, nbCol, cx, cy);
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
...
JNIEXPORT void JNICALL Java_javasci_Scilab_receiveComplexMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	strErr = readNamedComplexMatrixOfDouble((char*)cname, &rows, &cols, NULL, NULL);
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
	...
	strErr = readNamedComplexMatrixOfDouble((char*)cname, &rows, &cols, cx, cy);
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
...
JNIEXPORT void JNICALL Java_javasci_Scilab_receiveStringMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	strErr = readNamedMatrixOfString((char*)cname, &rows, &cols, pLength, pStrings);
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
		(*env)->ReleaseStringUTFChars(env, jname , cname);
...
JNIEXPORT jobject JNICALL Java_javasci_Scilab_receiveDataByName (JNIEnv *env, jclass cl, jstring name)
{
	...
	strErr = getNamedVarType((char*)cname, &type); /* give type */
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
		return obj;
	...
	strErr = getNamedVarDimension((char*)cname, &Dimensions[0], &Dimensions[1]);
	if(strErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(strErr));
		return obj;
