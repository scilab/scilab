...
JNIEXPORT void JNICALL Java_javasci_Scilab_sendDoubleMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	sciErr = createNamedMatrixOfDouble(NULL, (char*)cname, nbRow, nbCol, matrix);
	if(sciErr.iErr)
	{
...
JNIEXPORT void JNICALL Java_javasci_Scilab_receiveDoubleMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	sciErr = readNamedMatrixOfDouble(NULL, (char*)cname, &rows, &cols, NULL);
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
	...
	sciErr = readNamedMatrixOfDouble(NULL, (char*)cname, &rows, &cols, matrix);
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
	}
...
JNIEXPORT void JNICALL Java_javasci_Scilab_sendComplexMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	sciErr = createNamedComplexMatrixOfDouble(NULL, (char*)cname, nbRow, nbCol, cx, cy);
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
...
JNIEXPORT void JNICALL Java_javasci_Scilab_receiveComplexMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	sciErr = readNamedComplexMatrixOfDouble(NULL, (char*)cname, &rows, &cols, NULL, NULL);
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
	...
	sciErr = readNamedComplexMatrixOfDouble(NULL, (char*)cname, &rows, &cols, cx, cy);
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
...
JNIEXPORT void JNICALL Java_javasci_Scilab_sendStringMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	createNamedMatrixOfString(NULL, (char*)cname, nbRow, nbCol, pStrings);
...
JNIEXPORT void JNICALL Java_javasci_Scilab_receiveStringMatrix (JNIEnv *env, jclass cl, jobject objMatrix)
{
	...
	sciErr = readNamedMatrixOfString(NULL, (char*)cname, &rows, &cols, pLength, pStrings);
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
		(*env)->ReleaseStringUTFChars(env, jname , cname);
...
JNIEXPORT jobject JNICALL Java_javasci_Scilab_receiveDataByName (JNIEnv *env, jclass cl, jstring name)
{
	...
	sciErr = getNamedVarType(NULL, (char*)cname, &type); /* give type */
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
		return obj;
	...
	sciErr = getNamedVarDimension(NULL, (char*)cname, &Dimensions[0], &Dimensions[1]);
	if(sciErr.iErr)
	{
		fprintf(stderr,"%s", getErrorMessage(sciErr));
		return obj;
	...
	case sci_matrix : 
		...
		if (isNamedVarComplex(NULL, (char*)cname)) 
		{
