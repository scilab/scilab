...
assert_checkerror ( cmd , "%s: The boundsmin vector is expected to have %d columns, but current shape is %d x %d",[],"optimsimplex_randbounds",2,1,3);
...
boundsmax = [5 5 5];
cmd = "newobj = optimsimplex_new ( ""randbounds"" , x0 , rosenbrock , boundsmin , boundsmax );";
assert_checkerror ( cmd , "%s: The boundsmax vector is expected to have %d columns, but current shape is %d x %d",[],"optimsimplex_randbounds",2,1,3);
