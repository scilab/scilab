...
std::map<std::string, uint64_t> GVN::getSymMap() const
{
...
std::ostream & operator<<(std::ostream & out, const GVN & gvn)
{
    out << "Constants:" << std::endl;
    for (const auto & p : gvn.mapi64)
    {
        out << "  " << p.first << " -> " << p.second.value << std::endl;
    }

    out << "Symbols:" << std::endl;
    for (const auto & p : gvn.maps)
    {
        out << "  " << p.first.getName().data() << " -> " << p.second.value << std::endl;
    }
    ...
    out << "OpValues:" << std::endl;
    for (const auto & p : gvn.mapv)
    {
        out << "  " << p.first << " -> " << p.second.value << ", P: " << p.second.poly->print(map) << std::endl;
    }
