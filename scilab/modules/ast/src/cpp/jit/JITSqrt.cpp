...
namespace jit
{
    ...
    void JITSqrt::S(const analysis::TIType & argType, JITScilabPtr & arg, const analysis::TIType & OType, JITScilabPtr & out, JITVisitor & jit)
    {
    ...
    void JITSqrt::S_d(const analysis::TIType & argType, JITScilabPtr & arg, const analysis::TIType & OType, JITScilabPtr & out, JITVisitor & jit)
    {
        ...
        /* if (x >= 0 || std::isnan(x)) { return (sqrt(x), 0); } else { return (0, sqrt(-x)); } */
	...
	llvm::Value * res_re_2 = zero;
	llvm::Value * res_im_2 = ret;
	builder.CreateBr(after);
