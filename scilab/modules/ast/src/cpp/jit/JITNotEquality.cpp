...
namespace jit
{
    ...
    JITScilabPtr JITNotEquality::SS(JITScilabPtr & L, const analysis::TIType & Ltype, JITScilabPtr & R, const analysis::TIType & Rtype, const analysis::TIType & Otype, JITVisitor & jit)
    {
        ...
        else if (Ltype.isfloating() && Rtype.isfloating())
        {
            if (Ltype.type == analysis::TIType::DOUBLE)
            {
                ...
                else
                {
                    ...
                    llvm::Value * cmp_r_im_0 = builder.CreateFCmpUNE(r_im, jit.getConstant<double>(0)); // R.im != 0
                    builder.CreateCondBr(cmp_r_im_0, after, bb1);

                    builder.SetInsertPoint(bb1);
                    llvm::Value * r_re = R->loadReal(jit);
                    llvm::Value * cmp_l_r_re = builder.CreateFCmpUNE(l, r_re); // L != R.re
                    builder.CreateBr(after);

                    builder.SetInsertPoint(after);
                    llvm::PHINode * res = builder.CreatePHI(jit.getTy<bool>(), 2);
                    res->addIncoming(jit.getBool(true), cur_block);
            ...
            else
            {
                if (Rtype.type == analysis::TIType::DOUBLE)
                {
                    ...
                    llvm::Value * cmp_l_im_0 = builder.CreateFCmpUNE(l_im, jit.getConstant<double>(0)); // L.im != 0
                    builder.CreateCondBr(cmp_l_im_0, after, bb1);

                    builder.SetInsertPoint(bb1);
                    llvm::Value * l_re = L->loadReal(jit);
                    llvm::Value * cmp_l_re_r = builder.CreateFCmpUNE(l_re, r); // L.re != R
                    builder.CreateBr(after);
		    ...
		    llvm::Value * l_re = L->loadReal(jit);
                    llvm::Value * r_re = R->loadReal(jit);
		    llvm::Value * cmp_re = builder.CreateFCmpUNE(l_re, r_re); // L.re != R.re
                    builder.CreateCondBr(cmp_re, after, bb1);

                    builder.SetInsertPoint(bb1);
		    llvm::Value * l_im = L->loadImag(jit);
		    llvm::Value * r_im = R->loadImag(jit);
                    llvm::Value * cmp_im = builder.CreateFCmpUNE(l_im, r_im); // L.im != R.im
                    builder.CreateBr(after);
