...
InternalType* compnoequal_M_E(T *_pL, U *_pR)
{
    ...
    tmp.push_back(_pL);
    tmp.push_back(_pR);
    std::string overloadName(Overload::buildOverloadName(Overload::getNameFromOper(ast::OpExp::ne), tmp, 1, true));
...
InternalType* compnoequal_LT_LT(T *_pL, U *_pR)
{
    if (_pL->getType() != GenericType::ScilabList || _pR->getType() != GenericType::ScilabList)
    {
        ...
        in.push_back(_pL);
        in.push_back(_pR);
        std::string overloadName(Overload::buildOverloadName(Overload::getNameFromOper(ast::OpExp::ne), in, 1, true));
        types::InternalType* pIT = symbol::Context::getInstance()->get(symbol::Symbol(overloadName));
        if (pIT)
        {
            return NULL;
