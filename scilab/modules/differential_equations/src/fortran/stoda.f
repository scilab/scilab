...
c initialize switching parameters.  meth = 1 is assumed initially. -----
      ...
      pdlast = 0.0d+0
      ratio = 5.0d+0
      call cfode (2, elco(1,1), tesco(1,1))
      do 10 i = 1,5
 10     cm2(i) = tesco(2,i)*elco(i+1,i)
      call cfode (1, elco(1,1), tesco(1,1))
      do 20 i = 1,12
...
c-----------------------------------------------------------------------
      ...
      irflag = 0
      pdh = max(abs(h)*pdlast,0.0000010d+0)
      if (rh*pdh*1.000010d+0 .lt. sm1(nq)) go to 178
      rh = sm1(nq)/pdh
      irflag = 1
 178  continue
      r = 1.0d+0
...
c-----------------------------------------------------------------------
 400  continue
      if (del .le. 100.0d+0*pnorm*uround) go to 450
      if (m .eq. 0 .and. meth .eq. 1) go to 405
      if (m .eq. 0) go to 402
      rm = 1024.0d+0
      if (del .le. 1024.0d+0*delp) rm = del/delp
      rate = max(rate,rm)
      crate = max(0.20d+0*crate,rm)
 402  dcon = del*min(1.0d+0,1.50d+0*crate)/(tesco(2,nq)*conit)
      if (dcon .gt. 1.0d+0) go to 405
      pdest = max(pdest,rate/abs(h*el(1)))
      if (pdest .ne. 0.0d+0) pdlast = pdest
      go to 450
 405  continue
      m = m + 1
...
c-----------------------------------------------------------------------
      ...
      if (dsm .gt. 100.0d+0*pnorm*uround .and. pdest .ne. 0.0d+0)
     1   go to 470
 ...
 470  continue
      exsm = 1.0d+0/dble(l)
      rh1 = 1.0d+0/(1.20d+0*dsm**exsm + 0.00000120d+0)
      rh1it = 2.0d+0*rh1
      pdh = pdlast*abs(h)
      if (pdh*rh1 .gt. 0.000010d+0) rh1it = sm1(nq)/pdh
      rh1 = min(rh1,rh1it)
      if (nq .le. mxords) go to 474
         nqm2 = mxords
         lm2 = mxords + 1
         exm2 = 1.0d+0/dble(lm2)
         lm2p1 = lm2 + 1
         dm2 = vmnorm (n, yh(1,lm2p1), ewt)/cm2(mxords)
         rh2 = 1.0d+0/(1.20d+0*dm2**exm2 + 0.00000120d+0)
         go to 476
 474  dm2 = dsm*(cm1(nq)/cm2(nq))
      rh2 = 1.0d+0/(1.20d+0*dm2**exsm + 0.00000120d+0)
      nqm2 = nq
 476  continue
...
c-----------------------------------------------------------------------
 480  continue
      ...
      if (mxordn .ge. nq) go to 484
         nqm1 = mxordn
         lm1 = mxordn + 1
         exm1 = 1.0d+0/dble(lm1)
         lm1p1 = lm1 + 1
         dm1 = vmnorm (n, yh(1,lm1p1), ewt)/cm1(mxordn)
         rh1 = 1.0d+0/(1.20d+0*dm1**exm1 + 0.00000120d+0)
         go to 486
 484  dm1 = dsm*(cm2(nq)/cm1(nq))
      rh1 = 1.0d+0/(1.20d+0*dm1**exsm + 0.00000120d+0)
      nqm1 = nq
      exm1 = exsm
 486  rh1it = 2.0d+0*rh1
      pdh = pdnorm*abs(h)
      if (pdh*rh1 .gt. 0.000010d+0) rh1it = sm1(nqm1)/pdh
      rh1 = min(rh1,rh1it)
      rh2 = 1.0d+0/(1.20d+0*dsm**exsm + 0.00000120d+0)
      if (rh1*ratio .lt. 5.0d+0*rh2) go to 488
      alpha = max(0.0010d+0,rh1)
      dm1 = (alpha**exm1)*dm1
      if (dm1 .le. 1000.0d+0*uround*pnorm) go to 488
...
c if meth = 1 and h is restricted by stability, bypass 10 percent test.
 620  if (meth .eq. 2) go to 622
      if (rh*pdh*1.000010d+0 .ge. sm1(newq)) go to 625
 622  if (kflag .eq. 0 .and. rh .lt. 1.10d+0) go to 610
 625  if (kflag .le. -2) rh = min(rh,0.20d+0)
c-----------------------------------------------------------------------
