...
switch ( VarType(1)) 
{
  case sci_strings :
	...
	{
		 next=(int *)MALLOC(sizeof(int)*(200)); /* !!! WHY 200 ??? !!! */
	}
     ...
     for (i = 0; i < Number_of_Matrix_One ;i++)
     {
		 ...
		 {
			Output_StringMatrix[i]=(char*)MALLOC(sizeof(char)*(1));
    ...
    if (Rhs >= 4)
    {
           ...
           {  
                ...
                {
                    //z = regexec(&out1, Output_StringMatrix[x], nmatch, pm, 0);
       ...
       for (x=0;x<Number_of_Matrix_One;x++)
       {
            ...
            do
            {
				...
				if (Output_StringMatrix[x]) 
				{
					...
					w=kmp(Output_StringMatrix[x],Input_StringMatrix_Two[0],pos,next);
					if (Output_StringMatrix[x]==NULL) 
					{
						Scierror(999,"%s : Error memory allocation.\r\n",fname);
						return 0;
				...
				if (w!=0)
				{
                   _replacedstr = newstr(Output_StringMatrix[x], w-1, w + (int)strlen(Input_StringMatrix_Two[0])-1,*Input_StringMatrix_Three);
					if (Output_StringMatrix[x]) FREE(Output_StringMatrix[x]);
					Output_StringMatrix[x] = _replacedstr;
				}
				pos=w;
          ...
          //  strncpy(cstk(loutIndex),Output_StringMatrix[x], lnumCol);
           // CreateVarFromPtr( Rhs+1,MATRIX_OF_STRING_DATATYPE, &lnumRow, &lnumCol, Output_StringMatrix );
          //  LhsVar(x+1) = Rhs+x+1 ;
       }
   }
   numRow   = Row_One ;
