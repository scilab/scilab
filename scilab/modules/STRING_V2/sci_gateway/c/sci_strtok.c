...
int C2F(sci_strtok) _PARAMS((char *fname,unsigned long fname_len))
{
	...
	if (Rhs == 2)
	{
		if (GetType(2) == sci_strings)
		{
			...
			if (m2n2 != 1)
			{
				...
				Scierror(999,_("%s : Second parameter must be a string.\n"),fname);
				return 0;
			}
		}
		else
		{
			Scierror(999,_("%s : Second parameter is a invalid type.\n"),fname);
			return 0;
		}
	}
	...
	if (GetType(1) == sci_strings)
	{
		...
		if (m1n1 != 1)
		{
			...
			freeArrayOfString(Input_strings2,m2n2);
			Scierror(999,_("%s : First parameter must be a string.\n"),fname);
			return 0;
		}
	}
	else
	{
		freeArrayOfString(Input_strings2,m2n2);
		Scierror(999,_("%s : Fisrt parameter is a invalid type.\n"),fname);
		return 0;
	}
