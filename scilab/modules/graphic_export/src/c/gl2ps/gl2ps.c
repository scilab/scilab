...
typedef float GL2PSxyz[3];
typedef float GL2PSplane[4];
...
typedef struct {
  /* General */
  int format, sort, options, colorsize, colormode, buffersize;
  char *title, *producer, *filename;
  BOOL boundary, blending;
  float *feedback, offset[2], lastlinewidth;
  int viewport[4], blendfunc[2], lastfactor;
  GL2PSrgba *colormap, lastrgba, threshold, bgcolor;
  unsigned short lastpattern;
...
typedef struct {
  ...
  void  (*beginViewport)(int viewport[4]);
  int (*endViewport)(void);
...
static int gl2psPrintPrimitives(void);
...
static void gl2psMsg(int level, const char *fmt, ...)
{
  va_list args;
...
static void gl2psListRealloc(GL2PSlist *list, int n)
{
  if(!list){
...
static GL2PSlist *gl2psListCreate(int n, int incr, int size)
{
  GL2PSlist *list;
...
static void *gl2psListPointer(GL2PSlist *list, int index)
{
  if(!list){
...
static BOOL gl2psSameColor(GL2PSrgba rgba1, GL2PSrgba rgba2)
{
...
static BOOL gl2psVertsSameColor(const GL2PSprimitive *prim)
{
  int i;
...
static BOOL gl2psSameColorThreshold(int n, GL2PSrgba rgba[],
                                         GL2PSrgba threshold)
{
  int i;
...
static float gl2psGetRGB(GL2PSimage *im, unsigned int x, unsigned int y,
                           float *red, float *green, float *blue)
{
  ...
  *blue = *pimag; pimag++;
  ...
  return (im->format == joglGL_RGBA()) ? *pimag : 1.0F;
}
...
static GL2PSimage *gl2psCopyPixmap(GL2PSimage *im)
{
  ...
  if(image->format == joglGL_RGBA())
  {
	size = image->height * image->width * 4 * sizeof(float);
  }else
  {
	size = image->height * image->width * 3 * sizeof(float);
  }

  image->pixels = (float*)gl2psMalloc(size);
...
static int gl2psAddText(int type, const char *str, const char *fontname, 
                          short fontsize, int alignment, float angle)
{
  ...
  joglGetCurrentRasterPositionValid(&valid);
  if(FALSE == valid) return GL2PS_SUCCESS; /* the primitive is culled */
  ...
  joglGetCurrentRasterColor(prim->verts[0].rgba);
  ...
  joglPassThrough(GL2PS_TEXT_TOKEN); /*ici*/
...
static BOOL gl2psSupportedBlendMode(int sfactor, int dfactor)
{
  /* returns TRUE if gl2ps supports the argument combination: only two
     blending modes have been implemented so far */

  if( (sfactor == joglGL_SRC_ALPHA() && dfactor == joglGL_ONE_MINUS_SRC_ALPHA()) || 
      (sfactor == joglGL_ONE() && dfactor == joglGL_ZERO()) )
    return TRUE;
  return FALSE;
}
...
static void gl2psAdaptVertexForBlending(GL2PSvertex *v)
{
  /* Transforms vertex depending on the actual blending function -
     ...
     alpha value is changed to 1.0 if source blending joglGL_ONE is
  ...
  if(gl2ps->blendfunc[0] == joglGL_ONE())
  {
	 v->rgba[3] = 1.0F;
  }
...
static BOOL gl2psSamePosition(GL2PSxyz p1, GL2PSxyz p2)
{
...
static float gl2psComparePointPlane(GL2PSxyz point, GL2PSplane plane)
{
...
static float gl2psPsca(float *a, float *b)
{
  return(a[0]*b[0] + a[1]*b[1] + a[2]*b[2]);
}

static void gl2psPvec(float *a, float *b, float *c)
{
  c[0] = a[1]*b[2] - a[2]*b[1];
  c[1] = a[2]*b[0] - a[0]*b[2];
  c[2] = a[0]*b[1] - a[1]*b[0];
...
static float gl2psNorm(float *a)
{
  return (float)sqrt(a[0]*a[0] + a[1]*a[1] + a[2]*a[2]);
}

static void gl2psGetNormal(float *a, float *b, float *c)
{
  float norm;
...
static void gl2psCreateSplitPrimitive(GL2PSprimitive *parent, GL2PSplane plane,
                                      GL2PSprimitive *child, short numverts,
...
static void gl2psAddIndex(short *index0, short *index1, short *nb, 
                          short i, short j)
{
  int k;
...
static short gl2psGetIndex(short i, short num)
{
  return (i < num - 1) ? i + 1 : 0;
}

static int gl2psTestSplitPrimitive(GL2PSprimitive *prim, GL2PSplane plane)
{
  int type = GL2PS_COINCIDENT;
  short i, j;
  float d[5]; 
...
static int gl2psSplitPrimitive(GL2PSprimitive *prim, GL2PSplane plane, 
                                 GL2PSprimitive **front, GL2PSprimitive **back)
{
  short i, j, in = 0, out = 0, in0[5], in1[5], out0[5], out1[5];
  int type;
  float d[5]; 
...
static int gl2psFindRoot(GL2PSlist *primitives, GL2PSprimitive **root)
{
  int i, j, count, best = 1000000, index = 0;
...
static BOOL gl2psGreater(float f1, float f2)
{
  if(f1 > f2) return TRUE;
  else return FALSE;
}

static BOOL gl2psLess(float f1, float f2)
{
  if(f1 < f2) return TRUE;
  else return FALSE;
}
...
static void gl2psTraverseBspTree(GL2PSbsptree *tree, GL2PSxyz eye, float epsilon,
                                 ...
                                 void (*action)(void *data), int inverse)
{
  ...
  if(TRUE == compare(result, epsilon)){
  ...
  else if(TRUE == compare(-epsilon, result)){ 
    gl2psTraverseBspTree(tree->front, eye, epsilon, compare, action, inverse);
    if(inverse){
      gl2psListActionInverse(tree->primitives, action);
    }
...
{
  GL2PSprimitive *prim;
  float minZ, maxZ, rangeZ, scaleZ;
  float factor, units, area, dZ, dZdX, dZdY, maxdZ;
  int i, j;
...
static int gl2psGetPlaneFromPoints(GL2PSxyz a, GL2PSxyz b, GL2PSplane plane)
{
  float n; 

  plane[0] = b[1] - a[1];
  plane[1] = a[0] - b[0];
  n = (float)sqrt(plane[0]*plane[0] + plane[1]*plane[1]);
  plane[2] = 0.0F;
  if(!GL2PS_ZERO(n)){
    plane[0] /= n;
    plane[1] /= n;
...
static int gl2psCheckPoint(GL2PSxyz point, GL2PSplane plane)
{
  float pt_dis;
...
static int gl2psCheckPrimitive(GL2PSprimitive *prim, GL2PSplane plane)
{
  int i;
  int pos;
                                  ...
                                  GL2PSprimitive **back)
{
  ...
  int cur = -1, prev = -1, i, v1 = 0, v2 = 0, flag = 1, prev0 = -1;
  ...
  short front_count = 0, back_count = 0;
...
static int gl2psAddInBspImageTree(GL2PSprimitive *prim, GL2PSbsptree2d **tree)
{
  int ret = 0;
  ...
  if(*tree == NULL){
    if((prim->type != GL2PS_IMAGEMAP) && (FALSE == gl2ps->zerosurfacearea)){
    ...
    case GL2PS_COINCIDENT:
      if((*tree)->back != NULL){
        gl2ps->zerosurfacearea = TRUE;
        ret = gl2psAddInBspImageTree(prim, &(*tree)->back);
        gl2ps->zerosurfacearea = FALSE;
        if(ret) return ret;
      }
      if((*tree)->front != NULL){
        gl2ps->zerosurfacearea = TRUE;
...
static void gl2psAddPolyPrimitive(short type, short numverts, 
                                  GL2PSvertex *verts, int offset, 
                                  unsigned short pattern, int factor,
                                  float width, char boundary)
{
  GL2PSprimitive *prim;
...
static int gl2psGetVertex(GL2PSvertex *v, float *p)
{
  int i;
  ...
  if(gl2ps->colormode == joglGL_COLOR_INDEX() && gl2ps->colorsize > 0){
    i = (int)(p[3] + 0.5);
...
static void gl2psParseFeedbackBuffer(int used)
{
  char flag;
  unsigned short pattern = 0;
  BOOL boundary;
  int i, sizeoffloat, count, v, vtot, offset = 0, factor = 0, auxindex = 0;
  float lwidth = 1.0F, psize = 1.0F;
  float *current;
  GL2PSvertex vertices[3];
  GL2PSprimitive *prim;
  GL2PSimagemap *node;

  joglReadFeedbackBuffer(gl2ps->buffersize, gl2ps->feedback);

  current = gl2ps->feedback;
  boundary = gl2ps->boundary = FALSE;

  while(used > 0){

    if(TRUE == boundary) gl2ps->boundary = TRUE;
    
	if((int)*current == joglGL_POINT_TOKEN())
	{
	  current ++;
      used --;
	...
	}else if(((int)*current == joglGL_LINE_TOKEN()) || ((int)*current == joglGL_LINE_RESET_TOKEN()))
	{
	  current ++;
      used --;
      i = gl2psGetVertex(&vertices[0], current);
	...
	}else if((int)*current == joglGL_POLYGON_TOKEN())
	{
	  count = (int)current[1];
      current += 2;
      used -= 2;
	...
	}else if(((int)*current == joglGL_BITMAP_TOKEN()) || ((int)*current == joglGL_DRAW_PIXEL_TOKEN()) || ((int)*current == joglGL_COPY_PIXEL_TOKEN()))
	{
	  current ++;
      used --;
      i = gl2psGetVertex(&vertices[0], current);
	...
	}else if((int)*current == joglGL_PASS_THROUGH_TOKEN())
	{
      ...
      case GL2PS_BEGIN_STIPPLE_TOKEN : 
        current += 2;
        used -= 2; 
        pattern = (unsigned short)current[1]; 
        current += 2;
        used -= 2; 
        factor = (int)current[1]; 
        break;
      case GL2PS_SRC_BLEND_TOKEN : 
        current += 2; 
        used -= 2; 
        gl2ps->blendfunc[0] = (int)current[1];
        break;
      case GL2PS_DST_BLEND_TOKEN : 
        current += 2; 
        used -= 2; 
        gl2ps->blendfunc[1] = (int)current[1];
        break;
      ...
      case GL2PS_IMAGEMAP_TOKEN :
        ...
        node->image->pixels = (float*)gl2psMalloc(v + vtot);
...
static void gl2psPrintPostScriptImagemap(float x, float y,
...
static void gl2psParseStipplePattern(unsigned short pattern, int factor, 
                                     int *nb, int array[10])
{
  int i, n;
...
static int gl2psPrintPostScriptDash(unsigned short pattern, int factor, char *str)
{
...
static void gl2psPrintPostScriptBeginViewport(int viewport[4])
{
  int index;
  float rgba[4];
  int x = viewport[0], y = viewport[1], w = viewport[2], h = viewport[3];

  joglRenderMode(joglGL_FEEDBACK()); /*ici*/
  ...
  if(gl2ps->options & GL2PS_DRAW_BACKGROUND){
    if(gl2ps->colormode == joglGL_RGBA() || gl2ps->colorsize == 0){
	  joglGetColorClearValue(rgba);
    }
    else{
	  joglGetIndexClearValue(&index);	
...
static int gl2psPrintPostScriptEndViewport(void)
{
  int res;
...
static void gl2psPrintTeXBeginViewport(int viewport[4])
{
  joglRenderMode(joglGL_FEEDBACK()); /*ici*/
...
static int gl2psPrintTeXEndViewport(void)
{
...
static int gl2psPrintPDFLineWidth(float lw)
{
  if(GL2PS_ZERO(lw))
...
static void gl2psPDFgroupListInit(void)
{
  ...
  for(i = 0; i < gl2psListNbr(gl2ps->pdfprimlist); ++i){  
    ...
    case GL2PS_POINT:
      ...
      break;
    case GL2PS_TRIANGLE:
      gl2psFillTriangleFromPrimitive(&tmpt, p, TRUE);
...
static void gl2psSortOutTrianglePDFgroup(GL2PSpdfgroup *gro)
{
  ...
  gl2psFillTriangleFromPrimitive(&t, prim, TRUE);
  ...
  for(i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i){
    ...
    case GL2PS_TRIANGLE:
      ...
      if(t.prop & T_CONST_COLOR && t.prop & T_ALPHA_1){         
        ...
        for(j = 0; j <= lastel; ++j){  
          prim = *(GL2PSprimitive**)gl2psListPointer(gro->ptrlist, j);
          gl2psFillTriangleFromPrimitive(&t, prim, FALSE);
          gl2ps->streamlength 
            += gl2psPrintf("%f %f m\n"
                           "%f %f l\n"
                           "%f %f l\n"
...
static int gl2psPDFgroupListWriteXObjectResources(void)
{
  ...
  for(i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i){  
    ...
    case GL2PS_PIXMAP:
      ...
      if(joglGL_RGBA() == p->data.image->format)  /* reserve one object for image mask */
        gl2ps->objects_stack++;
...
static void gl2psPDFRectHull(float *xmin, float *xmax, 
...
static int gl2psPrintPDFShaderSimpleExtGS(int obj, float alpha)
{
  int offs = 0;
...
static int gl2psPrintPDFPixmap(int obj, int childobj, GL2PSimage *im, int gray)
{
  ...
  offs += fprintf(gl2ps->stream,
                  ...
                  (gray) ? "/DeviceGray" : "/DeviceRGB" );
  if(joglGL_RGBA() == im->format && gray == 0){
    offs += fprintf(gl2ps->stream,
                    "/SMask %d 0 R\n",
                    childobj);
  }
  ...
  for(i = 0; i < gl2psListNbr(gl2ps->pdfgrouplist); ++i){  
    ...
    case GL2PS_TRIANGLE:
      ...
      for(j = 0; j < size; ++j){  
        ...
        gl2psFillTriangleFromPrimitive(&triangles[j], p, TRUE);
      }
    ...
    case GL2PS_PIXMAP:
      ...
      if(p->data.image->format == joglGL_RGBA()){
...
static void gl2psPrintPDFBeginViewport(int viewport[4])
{
  int offs = 0;
  int index;
  float rgba[4];
  ...
  if(gl2ps->options & GL2PS_DRAW_BACKGROUND){
    if(gl2ps->colormode == joglGL_RGBA() || gl2ps->colorsize == 0){
      //glGetFloatv(joglGL_COLOR_CLEAR_VALUE(), rgba);
	  joglGetColorClearValue(rgba);
    }
    else{
      //glGetIntegerv(joglGL_INDEX_CLEAR_VALUE(), &index); /*ici*/
	  joglGetIndexClearValue(&index);
      rgba[0] = gl2ps->colormap[index][0];
      rgba[1] = gl2ps->colormap[index][1];
      rgba[2] = gl2ps->colormap[index][2];
...
static int gl2psPrintPDFEndViewport(void)
{
  int res;
...
static void gl2psPrintSVGDash(unsigned short pattern, int factor)
{
  int i, n, array[10];
...
static void gl2psPrintSVGPixmap(float x, float y, GL2PSimage *pixmap)
{
#if defined(GL2PS_HAVE_LIBPNG)
  GL2PSlist *png;
  unsigned char c;
...
static void gl2psPrintSVGBeginViewport(int viewport[4])
{
  int index;
  char col[32];
  float rgba[4];
...
static int gl2psPrintSVGEndViewport(void)
{
  int res;
...
static void gl2psPrintPGFDash(unsigned short pattern, int factor)
{
  int i, n, array[10];
...
static void gl2psPrintPGFBeginViewport(int viewport[4])
{
  int index;
  float rgba[4];
  int x = viewport[0], y = viewport[1], w = viewport[2], h = viewport[3];
...
static int gl2psPrintPGFEndViewport(void)
{
  int res;
  res = gl2psPrintPrimitives();
...
static void gl2psComputeTightBoundingBox(void *data)
{
  ...
  for(i = 0; i < prim->numverts; i++){
    if(prim->verts[i].xyz[0] < gl2ps->viewport[0])
      gl2ps->viewport[0] = (int)prim->verts[i].xyz[0];
    if(prim->verts[i].xyz[0] > gl2ps->viewport[2])
      gl2ps->viewport[2] = (int)(prim->verts[i].xyz[0] + 0.5F);
    if(prim->verts[i].xyz[1] < gl2ps->viewport[1])
      gl2ps->viewport[1] = (int)prim->verts[i].xyz[1];
    if(prim->verts[i].xyz[1] > gl2ps->viewport[3])
      gl2ps->viewport[3] = (int)(prim->verts[i].xyz[1] + 0.5F);
  }
}  
...
static int gl2psPrintPrimitives(void)
{
  ...
  used = joglRenderMode(joglGL_RENDER()); /*ici*/
  ...
  case GL2PS_BSP_SORT :
    ...
    if(TRUE == gl2ps->boundary) gl2psBuildPolygonBoundary(root);
    if(gl2ps->options & GL2PS_OCCLUSION_CULL){
                                  ...
                                  FILE *stream, const char *filename)
{
  ...
  if(format >= 0 && format < (int)(sizeof(gl2psbackends)/sizeof(gl2psbackends[0]))){
    gl2ps->format = format;
  }
  else {
  ...
  if(gl2ps->options & GL2PS_USE_CURRENT_VIEWPORT){
	joglGetViewport(gl2ps->viewport);
  }
  else{
    for(i = 0; i < 4; i++){
  ...
  gl2ps->threshold[0] = nr ? 1.0F/(float)nr : 0.064F;
  gl2ps->threshold[1] = ng ? 1.0F/(float)ng : 0.034F;
  gl2ps->threshold[2] = nb ? 1.0F/(float)nb : 0.100F;
  ...
  /* get default blending mode from current OpenGL state (enabled by
     default for SVG) */
  gl2ps->blending = (gl2ps->format == GL2PS_SVG) ? TRUE : joglIsEnabled(joglGL_BLEND()); /*ici*/
  joglGetBlendSrc(&gl2ps->blendfunc[0]);
  joglGetBlendDst(&gl2ps->blendfunc[1]);

  if(gl2ps->colormode == joglGL_RGBA()){
    gl2ps->colorsize = 0;
    gl2ps->colormap = NULL;
	joglGetColorClearValue(gl2ps->bgcolor);
  }
  else if(gl2ps->colormode == joglGL_COLOR_INDEX()){
    if(!colorsize || !colormap){
      gl2psMsg(GL2PS_ERROR, "Missing colormap for joglGL_COLOR_INDEX rendering");
      gl2psFree(gl2ps);
      gl2ps = NULL;
      return GL2PS_ERROR;
    }
  ...
  gl2ps->feedback = (float*)gl2psMalloc(gl2ps->buffersize * sizeof(float));
  joglFeedbackBuffer(gl2ps->buffersize, joglGL_3D_COLOR()); /*ici*/
  joglRenderMode(joglGL_FEEDBACK());  /*ici*/
                                   ...
                                   const void *pixels)
{
  ...
  if((format != joglGL_RGB() && format != joglGL_RGBA()) || type != joglGL_FLOAT()){
    gl2psMsg(GL2PS_ERROR, "gl2psDrawPixels only implemented for "
             "joglGL_RGB/joglGL_RGBA, joglGL_FLOAT pixels");
    return GL2PS_ERROR;
  }
  ...
  if(format == joglGL_RGBA())
  {
      if(gl2ps->options & GL2PS_NO_BLENDING || !gl2ps->blending){
		/* special case: blending turned off */
		prim->data.image->format = joglGL_RGB();
		size = height * width * 3;
		prim->data.image->pixels = (float*)gl2psMalloc(size * sizeof(float));
		 piv = (float*)pixels;
		...
		size = height * width * 4;
		prim->data.image->pixels = (float*)gl2psMalloc(size * sizeof(float));
		memcpy(prim->data.image->pixels, pixels, size * sizeof(float));
	  }
  }else
  {
	size = height * width * 3;
  ...
  gl2psListAdd(gl2ps->auxprimitives, &prim);
  joglPassThrough(GL2PS_DRAW_PIXELS_TOKEN); /*ici*/
...
GL2PSDLL_API int gl2psDrawImageMap(int width, int height,
  ...
  size = height + height * ((width-1)/8);
  joglPassThrough(GL2PS_IMAGEMAP_TOKEN); /*ici*/
  joglBegin(joglGL_POINTS()); /*ici*/
  joglVertex3f(position[0], position[1],position[2]); /*ici*/
  joglEnd(); /*ici*/
  joglPassThrough((float)width); /*ici*/
  joglPassThrough((float)height); /*ici*/
...
GL2PSDLL_API int gl2psEnable(int mode)
{
  ...
  if(mode == GL2PS_POLYGON_OFFSET_FILL)
  {
    joglPassThrough(GL2PS_BEGIN_OFFSET_TOKEN); /*ici*/
	joglGetPolygonOffsetFactor(&gl2ps->offset[0]);
	joglGetPolygonOffsetUnits(&gl2ps->offset[1]);
  }else if (mode == GL2PS_POLYGON_BOUNDARY)
  {
    joglPassThrough(GL2PS_BEGIN_BOUNDARY_TOKEN);
  ...
  {
	joglPassThrough(GL2PS_BEGIN_BLEND_TOKEN);
...
GL2PSDLL_API int gl2psDisable(int mode)
{
  ...
  if(mode == GL2PS_POLYGON_OFFSET_FILL)
  {
	joglPassThrough(GL2PS_END_OFFSET_TOKEN); 
  }else if(mode == GL2PS_POLYGON_BOUNDARY)
  {
	joglPassThrough(GL2PS_END_BOUNDARY_TOKEN);
  }else if(mode == GL2PS_LINE_STIPPLE)
  {
	joglPassThrough(GL2PS_END_STIPPLE_TOKEN); 
  }else if(mode == GL2PS_BLEND)
  {
	joglPassThrough(GL2PS_END_BLEND_TOKEN);
  }else{
...
GL2PSDLL_API int gl2psLineWidth(float value)
{
  ...
  joglPassThrough(GL2PS_LINE_WIDTH_TOKEN); /*ici*/
  joglPassThrough(value); /*ici*/
...
GL2PSDLL_API int gl2psBlendFunc(int sfactor, int dfactor)
{
  ...
  if(FALSE == gl2psSupportedBlendMode(sfactor, dfactor))
    return GL2PS_WARNING;

  joglPassThrough(GL2PS_SRC_BLEND_TOKEN); /*ici*/
  joglPassThrough((float)sfactor); /*ici*/
  joglPassThrough(GL2PS_DST_BLEND_TOKEN); /*ici*/
  joglPassThrough((float)dfactor); /*ici*/
