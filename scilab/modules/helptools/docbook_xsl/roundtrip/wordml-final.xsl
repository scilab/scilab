...
<!ENTITY itemizedlist 'w:p[starts-with(w:pPr/w:pStyle/@w:val,"d:itemizedlist") or
		       (w:pPr/w:listPr/w:ilvl/@w:val = "0" and
		       w:pPr/w:listPr/wx:t/@wx:val = "&#183;" and
		       w:pPr/w:listPr/wx:font/@wx:val = "Symbol")]'>
<!ENTITY itemizedlist1 'w:p[w:pPr/w:pStyle/@w:val = "d:itemizedlist1" or
			(w:pPr/w:listPr/w:ilvl/@w:val = "0" and
			w:pPr/w:listPr/wx:t/@wx:val = "&#183;" and
			w:pPr/w:listPr/wx:font/@wx:val = "Symbol")]'>
...
xmlns:w="http://schemas.microsoft.com/office/word/2003/wordml"
  ...
  exclude-result-prefixes="aml w wx o v d">
  ...
  <xsl:output indent="yes" method="xml" 
    cdata-section-elements='d:programlisting d:literallayout'/>
  ...
  <xsl:template match="wx:sub-section" mode="group">
    ...
    <xsl:choose>
      <xsl:when test='$element.name = "d:bogus"'>
        <xsl:apply-templates mode='group'/>
      </xsl:when>
      <xsl:when test='$element.name = "d:bibliography" or
		      $element.name = "d:bibliodiv" or
		      $element.name = "d:glossary" or
		      $element.name = "d:glossdiv" or
		      $element.name = "d:qandaset" or
		      $element.name = "d:qandadiv"'>
	<xsl:element name='{$element.name}'>
	  ...
	  <xsl:variable name='entries'
			select='*[1]/following-sibling::w:p[(starts-with($element.name, "d:biblio") and self::&biblioentrytitle;) or
				(starts-with($element.name, "d:gloss") and self::&glossterm;) or
				(starts-with($element.name, "d:qanda") and self::&question;)]'/>
  ...
  <xsl:template match="wx:sub-section/w:p[1]" mode="group">
    ...
    <xsl:choose>
      <xsl:when test='$parent != "d:qandaset" and
		      (../&releaseinfo; |
		      ../&author; |
		      ../&editor; |
		      ../&othercredit; |
  ...
  <xsl:template match='w:p[w:pPr/w:pStyle/@w:val = "d:highlights"]' mode='highlights'>
    <para>
      <xsl:call-template name='object.id'/>
      <xsl:apply-templates select='w:r|w:hlink'/>
    </para>
  ...
  <xsl:template match='w:p[w:pPr/w:pStyle/@w:val = "highlights-itemizedlist" or
    ...
    <xsl:if test='not(preceding-sibling::*) or
		  ...
		  preceding-sibling::*[1][self::w:p and w:pPr/w:pStyle/@w:val = "d:highlights"]'>
      <xsl:variable name='stop.node'
  ...
  <xsl:template match='w:r[starts-with(w:rPr/w:rStyle/@w:val, "d:emphasis")]'
		priority='2'>
    <xsl:choose>
  ...
  <xsl:template name='make-areaspec'>
    ...
    <xsl:if test='$areaspec'>
      ...
      <xsl:variable name='not.area'
		    select='$nodes[not(self::w:p) or self::w:p/w:pPr/w:pStyle/@w:val != "d:area"]'/>
...
<xsl:template match="w:p" mode="variablelist.term">
  ...
  <xsl:template name='attributes'>
    ...
    <xsl:variable name='attr'
      select='$node/w:r[w:rPr/w:rStyle/@w:val = "d:attributes"]'/>
  ...
  <xsl:template match='w:r[w:rPr/w:rStyle/@w:val = "d:attributes"]'/>
