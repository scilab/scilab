...
xmlns:sfa="http://developer.apple.com/namespaces/sfa"
  ...
  <!-- ********************************************************************
       $Id: pages-normalise.xsl 6910 2007-06-28 23:23:30Z xmldoc $
       ********************************************************************
  ...
  <xsl:template name='make-attributes'>
	...
	<xsl:if test='key("d:styles", $node/following-sibling::*[1]/@sf:style)/self::sf:characterstyle/@sf:name = "attribute-value"'>
	  <w:r>
	    <w:rPr>
	  ...
	  <xsl:if test='$node/following-sibling::node()[2][self::sf:span] and
			key("d:styles", $node/following-sibling::*[2]/@sf:style)/self::sf:characterstyle/@sf:name = "attribute-name"'>
	    <xsl:call-template name='make-attributes'>
  ...
  <xsl:template name='find-style'>
	...
	<w:rPr>
	  ...
	  <xsl:apply-templates select='key("d:styles", $ident)/self::sf:characterstyle/sf:property-map/*'
			       mode='styles'/>
	</w:rPr>
      </xsl:when>
    </xsl:choose>
