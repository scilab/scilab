...
/*------------------------------------------------------------------------*/
int set_y_shift_property(void* _pvCtx, int iObjUID, void* _pvData, int valueType, int nbRow, int nbCol)
{
    BOOL result = FALSE;
    ...
    if (nbElement != 0)
    {
        ...
        result = setGraphicObjectProperty(iObjUID, __GO_DATA_MODEL_Y_COORDINATES_SHIFT__, shiftCoordinates, jni_double_vector, iNumElements);
    ...
    else
    {
         ...
         */
        int shiftSet = 0;
        setGraphicObjectProperty(iObjUID, __GO_DATA_MODEL_Y_COORDINATES_SHIFT_SET__, &shiftSet, jni_double_vector, 1);
    }
