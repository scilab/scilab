...
if rhs==1 then
  if (type(r)<>1 & size(r,"*")<>3) then
    error("rgb2name: argument must be a vector of 3 integers")
  end
  b=r(3); g=r(2); r=r(1);
elseif rhs==3 then
  if (type(r)<>1 | type(g)<>1 | type(b)<>1 | size(r,"*")<>1 | size(g,"*")<>1 | size(b,"*")<>1) then
    error("rgb2name: arguments must be integers")
  end
else
  error("function ""name2rgb"" must have 1 or 3 arguments")
end
r=round(r); g=round(g); b=round(b);
if (r>255 | r<0 | g>255 | g<0 | b>255 | b<0) then
  error("rgb2name: arguments must be integers between 0 and 255")
end
