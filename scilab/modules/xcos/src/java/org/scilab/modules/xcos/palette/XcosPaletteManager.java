...
public class XcosPaletteManager {
    ...
    private static final JTabbedPane allpalettes = new JTabbedPane();
	...
	/**
	 * Background worker for populate the palette UI 
	 */
	private static class UpdatePaletteWorker extends
			SwingWorker<Void, PaletteDescriptor> {
		@Override
		protected Void doInBackground() throws Exception {
			for (PaletteStringDescriptor currentStringDescriptor : allPalettesStringDescriptor) {
				PaletteDescriptor currentDescriptor = new PaletteDescriptor(
						currentStringDescriptor.Name,
						createPaletteData(currentStringDescriptor.Components));
				publish(currentDescriptor);
			}
			return null;
		}

		@Override
		protected void process(List<PaletteDescriptor> chunks) {
			XcosPalette xcosPalette;

			for (PaletteDescriptor current : chunks) {
				xcosPalette = new XcosPalette();
				for (PaletteBlockData paletteBlockData : current.Components) {
					xcosPalette.addTemplate(paletteBlockData.Name,
							paletteBlockData.Icon, paletteBlockData.Block);
				}
	...
	/**
	 * Load all the palette
	 * @return A tab which contains or will contains the Palette instances.
	 */
	...
	private static PaletteBlockData[] createPaletteData(String[] blocksNames) {
		...
		final String blocksPath = System.getenv("SCI")
