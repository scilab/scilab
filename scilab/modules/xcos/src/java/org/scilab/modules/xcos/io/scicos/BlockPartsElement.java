...
abstract class BlockPartsElement extends AbstractElement<BasicBlock> {
	 ...
	 * 
	 * @param in the input port vector size
	 * @param out the output port vector size
	 * @param ein the event input (control) port vector size
	 * @param eout the event output (command) port vector size
	 */
	 ...
	 */
	protected void addSizedPortVector(ScilabMList element, Class< ? extends ScilabType> klass, int size) {
		ScilabType data;
