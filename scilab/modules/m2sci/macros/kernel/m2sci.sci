...
if isdefinedvar("%nargin") | isdefinedvar("%nargout") then
  dcl=["";gettext("messages","m2sci_message_52");"[%nargout,%nargin] = argn(0)"]
end

// Set display mode equivalent to Matlab echo off
dcl=[dcl;"";gettext("messages","m2sci_message_53");"mode(0);"]
// Set flotting point exception mode
dcl=[dcl;"";gettext("messages","m2sci_message_54");"ieee(1);"]
...
// Graphics init
//graph_ini=[gettext("messages","m2sci_message_55");"global %graphics";"%graphics.type=1";"%graphics.units=""pixels"""];
graph_ini=[]
if ini<>[] then
  ini=["";gettext("messages","m2sci_message_56");ini]
end
//ini=[ini;" ";graph_ini]

// Info on macros variables 
if verbose_mode<0 then
  write(%io(2),gettext("messages","m2sci_message_51"))
  n=size(varslist)
  info=[]
...
if batch then
  trad=[
      ...
      msprintf(gettext("messages","m2sci_message_57"),fnam)
      ""
  ...
  trad=[
      ...
      msprintf(gettext("messages","m2sci_message_58"),nam)
      ]
  ...
  elseif maclhs==1 then // Function with one output
    ...
    trad=[trad;
	gettext("messages","m2sci_message_59")
	dims
	gettext("messages","m2sci_message_60")
	vtype
	gettext("messages","m2sci_message_61")
	prop]
    if mtlbtree.outputs($).name<>"varargout" then
      trad=[trad;
	  "for k=1:lhs"
	  ...
	  "  tree.lhs(k).property=prop(k)"
	  "end"
	  gettext("messages","m2sci_message_62")
