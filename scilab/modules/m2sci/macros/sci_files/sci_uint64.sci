...
function tree = sci_uint64(tree)
    // Prototype for *int**()
    // M2SCI function
    // Conversion function for Matlab uint64()
    // Input: tree = Matlab funcall tree
    // Output: tree = Scilab equivalent for tree
    // Emulation function: mtlb_uint64()
    ...
    if x.vtype == String then
        // %c_uint64() is not defined in Scilab
        x = convert2double(x)
    end
    ...
    if x.type.property==Complex then
        ...
        set_infos(msprintf(msg,"uint64","uint64"),2);
    ...
    elseif typin.vtype==Boolean
        ...
        tree.lhs(1).type = Type(Int,"uint64")
        return

    elseif typin.vtype==Int
        if typin.property <> Unknown
            if part("uint64",1)==part(typin.property,1) // uint# => uint# | int# => int#
                [?,?,itin] = regexp(typin.property, "/[1-8]{1,}/");
                itin = evstr(itin);
                [?,?,itout] = regexp("uint64", "/[1-8]{1,}/");
                itout = evstr(itout);
                if itout >= itin
        ...
        if typeof(x)=="cste"
            v = x.value
            if min(v) >= uint64(-%inf) & max(v) <= uint64(%inf)
                tree.lhs(1).dims = x.dims
                tree.lhs(1).type = Type(Int,"uint64")
                return
            end
    ...
    elseif  typin==Type(Double,Real) & typeof(x)=="cste" // known value
        v = x.value
        if v==int(v) & min(v) >= uint64(-%inf) & max(v) <= uint64(%inf)
            tree.lhs(1).dims = x.dims
            tree.lhs(1).type = Type(Int,"uint64")
            return
        end
    end
    tree.name = "mtlb_uint64";
    tree.lhs(1).dims = x.dims;
    tree.lhs(1).type = Type(Int,"uint64");
endfunction
