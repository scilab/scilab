...
function tree = sci_uint16(tree)
    ...
    if x.vtype == String then
        // %c_uint16() is not defined in Scilab
        x = convert2double(x)
    end
    ...
    if x.type.property==Complex then
        ...
        set_infos(msprintf(msg,"uint16","uint16"),2);
    ...
    elseif typin.vtype==Boolean
        ...
        tree.lhs(1).type = Type(Int,"uint16")
        return

    elseif typin.vtype==Int
        if typin.property <> Unknown
            if part("uint16",1)==part(typin.property,1) // uint# => uint# | int# => int#
                [?,?,itin] = regexp(typin.property, "/[1-8]{1,}/");
                itin = evstr(itin);
                [?,?,itout] = regexp("uint16", "/[1-8]{1,}/");
                itout = evstr(itout);
                if itout >= itin
        ...
        if typeof(x)=="cste"
            v = x.value
            if min(v) >= uint16(-%inf) & max(v) <= uint16(%inf)
                tree.lhs(1).dims = x.dims
                tree.lhs(1).type = Type(Int,"uint16")
                return
            end
    ...
    elseif  typin==Type(Double,Real) & typeof(x)=="cste" // known value
        v = x.value
        if v==int(v) & min(v) >= uint16(-%inf) & max(v) <= uint16(%inf)
            tree.lhs(1).dims = x.dims
            tree.lhs(1).type = Type(Int,"uint16")
            return
        end
