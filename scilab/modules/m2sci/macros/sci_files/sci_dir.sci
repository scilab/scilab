...
function tree = sci_dir(tree)
    ...
    if tree.lhs(1).name=="ans" then
        ...
        tree.lhs(1).contents.data(1) = Infer(list(1,Unknown),Type(String, Char))

        tree.lhs(1).contents.index(2) = list(list(Cste("*"),Cste("*")),Cste("date"))
        tree.lhs(1).contents.data(2) = Infer(list(1,Unknown),Type(String, Char))
        ...
        tree.lhs(1).contents.data(4) = Infer(list(1,1),Type(Boolean,Boolean))
    end
