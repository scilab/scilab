...
function tree = sci_int64(tree)
    // Prototype for *int**()
    // M2SCI function
    // Conversion function for Matlab int64()
    // Input: tree = Matlab funcall tree
    // Output: tree = Scilab equivalent for tree
    // Emulation function: mtlb_int64()
    ...
    if x.vtype == String then
        // %c_int64() is not defined in Scilab
        x = convert2double(x)
    end
    ...
    if x.type.property==Complex then
        ...
        set_infos(msprintf(msg,"int64","int64"),2);
    ...
    elseif typin.vtype==Boolean
        ...
        tree.lhs(1).type = Type(Int,"int64")
        return

    elseif typin.vtype==Int
        if typin.property <> Unknown
            if part("int64",1)==part(typin.property,1) // uint# => uint# | int# => int#
                [?,?,itin] = regexp(typin.property, "/[1-8]{1,}/");
                itin = evstr(itin);
                [?,?,itout] = regexp("int64", "/[1-8]{1,}/");
                itout = evstr(itout);
                if itout >= itin
        ...
        if typeof(x)=="cste"
            v = x.value
            if min(v) >= int64(-%inf) & max(v) <= int64(%inf)
                tree.lhs(1).dims = x.dims
                tree.lhs(1).type = Type(Int,"int64")
                return
            end
    ...
    elseif  typin==Type(Double,Real) & typeof(x)=="cste" // known value
        v = x.value
        if v==int(v) & min(v) >= int64(-%inf) & max(v) <= int64(%inf)
            tree.lhs(1).dims = x.dims
            tree.lhs(1).type = Type(Int,"int64")
            return
        end
    end
    tree.name = "mtlb_int64";
    tree.lhs(1).dims = x.dims;
    tree.lhs(1).type = Type(Int,"int64");
endfunction
