 		...
 		if (Rhs == 1) then
 		  if (IsAScalar(varargin(1))==%T) then
    		h=TCL_scf(varargin(1));
 		    ...
 		    error(gettext('errors','tclsci_error_48'),999);
 		  end
 		else
 		  if (modulo(Rhs,2)==0) then
					...
					for i=1:2:Rhs
					  if ( type(varargin(i+1)) <> GetUiControlValueType(GetUiControlPropertyName(varargin(i))) ) then
					    if ( GetUiControlValueType(GetUiControlPropertyName(varargin(i))) == 10) then
					      ...
					      error(string(varargin(i+1))+gettext('errors','tclsci_error_46'),999);
					    end
					  else
 		    ...
 		    if (IsAScalar(varargin(1))==%T) then
    		  ...
    		  for i=2:2:Rhs
					   if ( type(varargin(i+1)) <> GetUiControlValueType(GetUiControlPropertyName(varargin(i))) ) then
					     if ( GetUiControlValueType(GetUiControlPropertyName(varargin(i))) == 10) then
					       error(string(varargin(i+1))+gettext('errors','tclsci_error_49'),999);
					     else
					       error(string(varargin(i+1))+gettext('errors','tclsci_error_46'),999);
					     end
					   else
