 ...
 */
public final class GraphicObjectMethodFinder {
    ...
    public static Method findSetter(String name, Class<?> baseClass, Object value) {
    ...
    public static Method findSetter(final String name, final Class<?> baseClass, final ScilabType arg) {
        ...
        if (methods != null) {
            final Class <? extends ScilabType > argClass = arg.getClass();
            ...
            for (Method m : methods) {
                ...
                Class <? extends ScilabType > st = ScilabTypeConverters.getPreferred().get(argType);
                if (st == argClass) {
                    if (!argType.isArray()) {
                        if (r == 1 && c == 1) {
                            return m;
                ...
                if (Modifier.isPublic(modifiers)) {
                    ...
                    if (len == 0) {
                        if (name.startsWith("get") || name.startsWith("is")) {
                            MethodsInfo.add(name, m, info.getters);
                        }
                    } else {
                        if (len == 1) {
                            if (name == add) {
                                checkMethod(m);
                                info.adders.add(m);
                            } else if (name.startsWith("set") && (ScilabTypeConverters.containConverter(argsTypes[0]) || StringConverters.containConverter(argsTypes[0]) || ScilabType.class.isAssignableFrom(argsTypes[0]))) {
                                MethodsInfo.add(name, m, info.setters);
                            }
                        }
                    }
                }
