...
void usbDConfigPort_USBTC(int fd, __u8 direction)
{
  __u8 cmd[1];
...
void usbDConfigBit_USBTC(int fd, __u8 bit_num, __u8 direction)
{
  __u8 cmd[2];
...
/* reads digital port  */
void usbDIn_USBTC(int fd, __u8* value)
{
...
/* reads digital bit  */
void usbDInBit_USBTC(int fd, __u8 bit_num, __u8* value)
{
  __u8 cmd[1];
...
/* writes digital port */
void usbDOut_USBTC(int fd, __u8 value)
{
  __u8 cmd[1];
...
/* writes digital bit  */
void usbDOutBit_USBTC(int fd, __u8 bit_num, __u8 value)
{
  __u8 cmd[2];
...
void usbTin_USBTC(int fd, __u8 channel, __u8 units, float *value)
{
...
void usbTinScan_USBTC(int fd, __u8 start_chan, __u8 end_chan, __u8 units, float value[])
{
  int nchan;
  __u8 cmd[3];
...
void usbReadMemory_USBTC(int fd, __u16 address, __u8 type, __u8 count, __u8 *memory)
{
  struct t_arg {
    __u16 address;
...
int usbWriteMemory_USBTC(int fd, __u16 address, __u8 type, __u8 count, __u8* data)
{
...
void usbSetItem_USBTC(int fd, __u8 item, __u8 subitem, __u32 value)
{
  int size;
  __u8 cmd[6];

  if ( item > 3 ) {
    printf("Error: usbSetItem_USBTC  Item = %d too large.\n", item);
  }
  ...
  switch (subitem) {
    ...
    case VREF:
      ...
      break;
    default:
      printf("Error usbSetItem_USBTC: subitem = %#x unknown\n", subitem);
      return;
  }
...
void usbGetItem_USBTC(int fd, __u8 item, __u8 subitem, void* value)
{
  int size;
  __u8 cmd[2];
...
void usbCalibrate_USBTC(int fd)
{
  /*
  ...
  */
  ...
  do {
    sleep(1);
    printf(".");
  } while ((usbGetStatus_USBTC(fd) & 0x1) == 1);
  printf("\n");
}

__u8  usbGetBurnoutStatus_USBTC(int fd, __u8 mask)
{
    /*
...
void usbPrepareDownload_USBTC(int fd, __u8 micro)
{
  /*
...
void usbWriteCode_USBTC(int fd, __u32 address, __u8 count, __u8 data[])
{
  /*
...
void usbWriteSerial_USBTC(int fd, __u8 serial[8])
{
...
int usbReadCode_USBTC(int fd, __u32 address, __u8 count, __u8 data[])
{
  struct t_arg {
    __u8 address[3];
    __u8 count;
