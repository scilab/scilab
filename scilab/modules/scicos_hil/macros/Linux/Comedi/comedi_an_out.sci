...
function [x,y,typ] = comedi_an_out(job,arg1,arg2)
//
...
case 'set' then //** set parameters 
  ...
  while %t do
       [ok,ch,name,com_range,aref,lab]=..
         getvalue('Set Comedi block parameters',..
         [ 'Channel';
           'Device';
	          'Range';
            'Aref'],..
...
case 'define' then      //** the standard define  
  ch=0; com_range=0; // default values 
  aref=0; 

  name='Comedi D/A' ; //** Show in the block 
  ...
  model.dstate=[]
  model.rpar=[]
  model.ipar=[ch com_range]; disp(model.ipar); 
  model.blocktype='c'
  model.firing=[]
  model.dep_ut=[%t %f]
  model.nzcross=0
