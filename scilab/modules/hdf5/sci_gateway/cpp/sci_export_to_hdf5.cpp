...
static bool export_data(int _iH5File, int* _piVar, char* _pstName)
{
	...
	SciErr strErr = getVarType(pvApiCtx, _piVar, &iType);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return false;
...
static bool export_list(int _iH5File, int *_piVar, char* _pstName, int _iVarType)
{
	...
	bool bReturn = false;
	int iItemNumber = 0;
	SciErr strErr = getListItemNumber(pvApiCtx, _piVar, &iItemNumber);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return false;
...
static bool export_poly(int _iH5File, int *_piVar, char* _pstName)
{
	...
	SciErr strErr = getPolyVariableName(pvApiCtx, _piVar, pstVarName, &iVarNameLen);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
...
static bool export_boolean(int _iH5File, int *_piVar, char* _pstName)
{
	...
	SciErr strErr = getMatrixOfBoolean(pvApiCtx, _piVar, &iRows, &iCols, &piData);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return 0;
...
static bool export_boolean_sparse(int _iH5File, int *_piVar, char* _pstName)
{
	...
	SciErr strErr = getBooleanSparseMatrix(pvApiCtx, _piVar, &iRows, &iCols, &iNbItem, &piNbItemRow, &piColPos);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return false;
...
static bool export_ints(int _iH5File, int *_piVar, char* _pstName)
{
	...
	SciErr strErr = getMatrixOfIntegerPrecision(pvApiCtx, _piVar, &iPrec);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
...
static bool export_strings(int _iH5File, int *_piVar, char* _pstName)
{
	...
	SciErr strErr = getMatrixOfString(pvApiCtx, _piVar, &iRows, &iCols, NULL, NULL);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
...
int extractVarNameList(int _iStart, int _iEnd, char** _pstNameList)
{
	...
	for(int i = _iStart ; i <= _iEnd ; i++)
	{
		...
		SciErr strErr = getVarAddressFromPosition(pvApiCtx, i, &piAddr);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
