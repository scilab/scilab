...
int sci_export_to_hdf5(char *fname,unsigned long fname_len)
{
	...
	{
		strErr = getVarAddressFromName(pstNameList[i + 1], &piAddrList[i]);
		if(strErr.iErr)
		{
...
static bool export_data(int _iH5File, int* _piVar, char* _pstName)
{
	...
	StrErr strErr = getVarType(_piVar, &iType);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return false;
...
static bool export_list(int _iH5File, int *_piVar, char* _pstName, int _iVarType)
{
	...
	bool bReturn = false;
	int iItemNumber = 0;
	StrErr strErr = getListItemNumber(_piVar, &iItemNumber);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return false;
...
static bool export_double(int _iH5File, int *_piVar, char* _pstName)
{
	...
	if(iComplex)
	{
		strErr = getComplexMatrixOfDouble(_piVar, &iRows, &iCols, &pdblReal, &pdblImg);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
	...
	{
		strErr = getMatrixOfDouble(_piVar, &iRows, &iCols, &pdblReal);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
...
static bool export_poly(int _iH5File, int *_piVar, char* _pstName)
{
	...
	StrErr strErr = getPolyVariableName(_piVar, pstVarName, &iVarNameLen);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
	...
	{
		strErr = getComplexMatrixOfPoly(_piVar, &iRows, &iCols, NULL, NULL, NULL);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		strErr = getComplexMatrixOfPoly(_piVar, &iRows, &iCols, piNbCoef, NULL, NULL);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		strErr = getComplexMatrixOfPoly(_piVar, &iRows, &iCols, piNbCoef, pdblReal, pdblImg);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
	...
	{
		strErr = getMatrixOfPoly(_piVar, &iRows, &iCols, NULL, NULL);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		piNbCoef = (int*)MALLOC(iRows * iCols * sizeof(int));
		strErr = getMatrixOfPoly(_piVar, &iRows, &iCols, piNbCoef, NULL);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		strErr = getMatrixOfPoly(_piVar, &iRows, &iCols, piNbCoef, pdblReal);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
...
static bool export_boolean(int _iH5File, int *_piVar, char* _pstName)
{
	...
	StrErr strErr = getMatrixOfBoolean(_piVar, &iRows, &iCols, &piData);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return 0;
...
static bool export_boolean_sparse(int _iH5File, int *_piVar, char* _pstName)
{
	...
	StrErr strErr = getBooleanSparseMatrix(_piVar, &iRows, &iCols, &iNbItem, &piNbItemRow, &piColPos);
	if(strErr.iErr)
	{
			printError(&strErr, 0);
			return false;
...
static bool export_sparse(int _iH5File, int *_piVar, char* _pstName)
{
	...
	if(isVarComplex(_piVar))
	{
		strErr = getComplexSparseMatrix(_piVar, &iRows, &iCols, &iNbItem, &piNbItemRow, &piColPos, &pdblReal, &pdblImg);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
	...
	{
		strErr = getSparseMatrix(_piVar, &iRows, &iCols, &iNbItem, &piNbItemRow, &piColPos, &pdblReal);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
...
static bool export_ints(int _iH5File, int *_piVar, char* _pstName)
{
	...
	StrErr strErr = getMatrixOfIntegerPrecision(_piVar, &iPrec);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
	...
	{
	case SCI_INT8 :
		strErr = getMatrixOfInteger8(_piVar, &iRows, &iCols, (char**)&piData);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		break;
	case SCI_UINT8 :
		strErr = getMatrixOfUnsignedInteger8(_piVar, &iRows, &iCols, (unsigned char**)&piData);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		break;
	case SCI_INT16 : 
		strErr = getMatrixOfInteger16(_piVar, &iRows, &iCols, (short**)&piData);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		break;
	case SCI_UINT16 :
		strErr = getMatrixOfUnsignedInteger16(_piVar, &iRows, &iCols, (unsigned short**)&piData);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		break;
	case SCI_INT32 : 
		strErr = getMatrixOfInteger32(_piVar, &iRows, &iCols, (int**)&piData);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		break;
	case SCI_UINT32 :
		strErr = getMatrixOfUnsignedInteger32(_piVar, &iRows, &iCols, (unsigned int**)&piData);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
...
static bool export_strings(int _iH5File, int *_piVar, char* _pstName)
{
	...
	StrErr strErr = getMatrixOfString(_piVar, &iRows, &iCols, NULL, NULL);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
	...
	strErr = getMatrixOfString(_piVar, &iRows, &iCols, piLen, NULL);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
	...
	strErr = getMatrixOfString(_piVar, &iRows, &iCols, piLen, pstData);
	if(strErr.iErr)
	{
		printError(&strErr, 0);
		return false;
...
int extractVarNameList(int _iStart, int _iEnd, char** _pstNameList)
{
	...
	for(int i = _iStart ; i <= _iEnd ; i++)
	{
		...
		StrErr strErr = getVarAddressFromPosition(i, &piAddr);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		//get filename
		strErr = getVarType(piAddr, &iType);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
		...
		strErr = getMatrixOfString(piAddr, &iRows, &iCols, &iLen, &_pstNameList[iCount]);
		if(strErr.iErr)
		{
			printError(&strErr, 0);
			return false;
