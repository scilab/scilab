...
function result = updateToolbox(name, checkVersionScilab)
  ...
  if name == "all"
    ...
    d = rep + name
    if ~isdir(d)
      atomsDisplayMessage("The toolbox " + name + " is not installed")
      result = %f
      return result
    end
    ...
    if position == 0
      atomsDisplayMessage("Toolbox not available.")
      result = %f
      return result
    end
    ...
    // We warm it
    versionNew = atomsExtractValue("Version", listDesc, position)
    atomsDisplayMessage("The toolbox " + name + " is going to be updated to version " + versionNew)
