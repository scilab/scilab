...
types::Function::ReturnValue sci_min(types::typed_list &in, int _iRetCount, types::typed_list &out)
{
    ...
    int iOrientation = 0;
    int iLargerInput = 0; // used to know the larger type of inputs arguments (in interger case). min(int16, int32, int8) return an int32
    ...
    {
        std::wstring wstFuncName = L"%"  + inputs[0]->getShortTypeStr() + L"_min";
    ...
    min(vectDouble, iOrientation, pDblIndex, pDblOut);
