...
/*----------------------------------------------------------------------------*/
/* get length */
static int lengthStrings(int* _piKey, int *piAddressVar);
static int lengthOthers(char *fname, int* _piKey);
static int lengthDefault(int* _piKey, int *piAddressVar);
static int lengthList(int* _piKey, int *piAddressVar);
/* !!! WARNING !!! : Read comments about length on sparse matrix */
static int lengthSparse(int* _piKey, int *piAddressVar);
/*----------------------------------------------------------------------------*/
int sci_length(char *fname, int* _piKey)
{
	...
	sciErr = getVarType(_piKey, piAddressVarOne, &iScilabType);
	if(sciErr.iErr)
	{
		printError(&sciErr, 0);
		return 0;
	...
	case sci_strings :
		{
			return lengthStrings(_piKey, piAddressVarOne);
		}
	case sci_sparse :
		{
			return lengthSparse(_piKey, piAddressVarOne);
		}
	case sci_list :
	case sci_tlist :
	case sci_mlist :
		{
			return lengthList(_piKey, piAddressVarOne);
		}
		break;
	...
	case sci_matlab_sparse : case sci_ints : case sci_handles : 
		{
			return lengthDefault(_piKey, piAddressVarOne);
		}
	default :
		return lengthOthers(fname, _piKey);
		break;
	}
}
/*--------------------------------------------------------------------------*/
static int lengthStrings(int* _piKey, int *piAddressVar)
{
	...
	sciErr = getMatrixOfString(_piKey, piAddressVar, &m1, &n1, lenStVarOne, pStVarOne);
	if(sciErr.iErr)
	{
		printError(&sciErr, 0);
	...
	sciErr = createMatrixOfDouble(_piKey, Rhs + 1, m_out, n_out, pdOut);
	if(sciErr.iErr)
	{
		printError(&sciErr, 0);
		return 0;
...
static int lengthSparse(int* _piKey, int *piAddressVar)
{
	...
	int m = 0, n = 0;
	SciErr sciErr = getVarDimension(_piKey, piAddressVar, &m, &n);
	if(sciErr.iErr)
	{
		printError(&sciErr, 0);
		return 0;
...
static int lengthList(int* _piKey, int *piAddressVar)
{
	...
	int nbItem = 0;
	SciErr sciErr = getListItemNumber(_piKey, piAddressVar, &nbItem);
	if(sciErr.iErr)
	{
		printError(&sciErr, 0);
		return 0;
