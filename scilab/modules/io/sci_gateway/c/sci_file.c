...
/*--------------------------------------------------------------------------*/
static int sci_file_no_rhs(char *fname, int* _piKey);
...
static int sci_file_no_rhs(char *fname, int* _piKey)
{
	...
	if (Lhs > 1) /* Types */
	{
		...
		if (TypeIdsAsString)
		{
			m_out = 1;
			n_out = sizeArray;
			sciErr = createMatrixOfString(_piKey, Rhs + 2, m_out, n_out, TypeIdsAsString);
	...
	if (Lhs > 2) /* names */
	{
		...
		if (Filenames)
		{
			m_out = 1;
			n_out = sizeArray;
			sciErr = createMatrixOfString(_piKey, Rhs + 3, m_out, n_out, Filenames);
