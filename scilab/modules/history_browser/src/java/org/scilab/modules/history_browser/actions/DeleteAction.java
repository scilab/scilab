...
public final class DeleteAction extends CallBack {
	...
	public void callBack() {
		TreePath currentSelection = CommandHistory.getTree().getSelectionPath();
		if (currentSelection != null) {
			...
			if (parent != null) {
				if (parent.getParent() == null) {
					...
					/* parent == rootNode */
					int linePosition = 0;
					for (int i = 0; i < parent.getIndex(currentNode); i++) {
						linePosition++; /* Session beginning line */
						linePosition += parent.getChildAt(i).getChildCount();
					}
					...
					for (int i = childCount; i > 0; i--) {
						HistoryManagement.deleteNthLineScilabHistory(linePosition + i);
					}
					...
					for (int i = 0; i < parentPosition; i++) {
						linePosition++; /* Session beginning line */
						linePosition += rootNode.getChildAt(i).getChildCount();
					}
