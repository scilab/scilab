...
while %t
  ...
  if %nok>0 then
    x_message(["answer given for  "+%labels(%nok);,"has invalid dimension: ";,"waiting for dimension  "+%ssz])
    %ini = %str
  elseif %nok<0 then
    if %ierr==0 then
      x_message(["answer given for  "+%labels(-%nok);,"has incorrect type :"+%typ(-2*%nok-1)])
    else
      x_message(["answer given for  "+%labels(-%nok);,"is incorrect see error message in scilab window"])
    end;
    %ini = %str
  else
    break;
...
if argn(2)<1 then
  Title = ["this is a demo";"sdsdfs sdfsdddddddddd w"+" errrereeeeeeeeeeeeeeeeeeeeee"]
