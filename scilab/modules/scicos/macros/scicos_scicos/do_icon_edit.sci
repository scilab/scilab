...
//
  ...
  win = max(winsid())+1; //** compute a new win_id
  fig = scf(win); //** open a new figure 
  xselect(); //** put in foreground 
  fig.axes_size = fig.axes_size*sz(1)/sz(2) ;
  fig.auto_resize = "off" ;
  ax = gca(); //** recover the axes
  if coli<>[] then
    ax.background = coli;
  end//background
  ...
  //** BEWARE : this code is now OBSOLETE ! 
  ...
  if and(win<>winsid()) then
    scf(gh_curwin); 
    return
  end //window destroyed
 ...
 delete(fig); //** close the icon editor window
