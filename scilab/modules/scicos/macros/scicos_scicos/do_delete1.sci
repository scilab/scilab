...
//**
  ...
  if gr == %t then
		...
		//** o_size(1) is the number of compound object
			...
			if or(fromblock.gui==['SPLIT_f' 'CLKSPLIT_f' 'IMPSPLIT_f']) then
				...
				if size(setdiff(connected,K),'*')==2 then
					...
					if find(connected(2)==DEL)<>[] then // delete split
						...
						scs_m.objs(to2(1))=mark_prt(scs_m.objs(to2(1)),to2(2),outin(to2(3)+1),ct2(2), connected(2))
						scs_m.objs(o1.from(1))=mark_prt(scs_m.objs(o1.from(1)),o1.from(2),outin(o1.from(3)+1),o1.ct(2),connected(2))
			...
			elseif toblock.gui=='IMPSPLIT_f' then
				...
				if size(connected,'*')==2 then
					...
					if find(connected(2)==DEL)<>[] then // delete split
						...
						if (o1.xx(1)==o1.xx(2)& o2.xx(1)==o2.xx(2))| (o1.yy(1)==o1.yy(2)& o2.yy(1)==o2.yy(2)) then
						...
						scs_m.objs(to1(1))=mark_prt(scs_m.objs(to1(1)),to1(2),outin(to1(3)+1),o1.ct(2),connected(2))
						scs_m.objs(to2(1))=mark_prt(scs_m.objs(to2(1)),to2(2),outin(to2(3)+1),ct(2),connected(2))
