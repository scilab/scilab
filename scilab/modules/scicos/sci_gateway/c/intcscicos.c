 ...
 if (ierr>0)
 {
  switch (ierr)
  {
   ...
   case 23  : strcpy(C2F(cha1).buf,"Cannot find the initial mode, maybe there is a sliding mode condition");
               break;
   case 24  : strcpy(C2F(cha1).buf,"You have changed a parameter in your model, but the model has been compiled to use an XML file containing initial values and parameters. So you should either recompile your Scicos diagram or [re]launch the initialization interface to regenerate the XML file  with new parameters.");
	   break;

   case 25  : strcpy(C2F(cha1).buf,"Undefined data type.");
	   break;
   case 26  : strcpy(C2F(cha1).buf,"The number of parameter provided by Scicos blocks is different from what expected by the code generated by the Modelica compiler. You might have relaxed a parameter using FIXED property (i.e., fixed=false) in a Modelica model. This will be corrected in the next version");
	   break;
   ...
   case 236  : strcpy(C2F(cha1).buf,"IDA_EWT_FAIL: The user-provide EwtSet function failed.");
               break;
   case 237  : strcpy(C2F(cha1).buf,"IDACalc cannot find the initial condition at this point. If you are using a Modelica model, you can try with the initialization GUI to try with other nonlinear solvers."); 
               break;
   ...
   case 256  : strcpy(C2F(cha1).buf,"IDA_CLOSE_ROOTS: Root found at and very near starting time.");
               break;
   case 257  : strcpy(C2F(cha1).buf,"IDA_Residual_NAN: The residual function returns NAN.");
	   break;
   case 258  : strcpy(C2F(cha1).buf,"IDA_Rootfunction_NAN: The Root function returns NAN.");
	   break;
   ...
   case 348  : strcpy(C2F(cha1).buf,"CV_CLOSE_ROOTS: Root found at and very near starting time.");
               break;
   case 349  : strcpy(C2F(cha1).buf,"CV_Derivative_NAN: The Derivatives returned by blocks are NAN.");
	   break;
   case 350  : strcpy(C2F(cha1).buf,"CV_Rootfunction_NAN: The Root function returns NAN.");
	   break;
   case 401  : strcpy(C2F(cha1).buf,"KIN_MEM_NULL: An error in the memory allocation in KINCreate or KINMalloc.");
	   break;
   case 402  : strcpy(C2F(cha1).buf,"KIN_ILL_INPUT: A supplied parameter is invalid (check error message).");
	   break;
   case 403  : strcpy(C2F(cha1).buf,"KIN_NO_MALLOC: Additional system memory has not yet been  allocated for vector storage (forgot to call the  KINMalloc routine)");
	   break;
   case 404  : strcpy(C2F(cha1).buf,"KIN_MEM_FAIL: An error occurred during memory allocation (either insufficient system resources are available or the vector kernel has not yet been initialized ");
	   break;
   case 405  : strcpy(C2F(cha1).buf,"KIN_LINESEARCH_NONCONV:  The line-search algorithm was unable to find an iterate sufficiently distinct  from the current iterate failure to satisfy the sufficient decrease condition could mean the current iterate is close to an approximate solution of the given nonlinear system, the finite-difference  approximation of the matrix-vector product  J(u)*v is inaccurate, or the real scalar scsteptol is too large ");
	   break;
   case 406  : strcpy(C2F(cha1).buf,"KIN_MAXITER_REACHED: The maximum number of nonlinear iterations has been reached ");
	   break;
   case 407  : strcpy(C2F(cha1).buf,"KIN_MXNEWT_5X_EXCEEDE: Five consecutive steps have been taken that satisfy the following inequality:  ||uscale*p||_L2 > 0.99*mxnewtstep where p denotes the current step and mxnewtstep is a real scalar upper bound on the scaled step length such a failure may mean ||fscale*func(u)||_L2 asymptotes from above to a finite value, or the real scalar mxnewtstep is too small ");
	   break;
   case 408 : strcpy(C2F(cha1).buf,"KIN_LINESEARCH_BCFAI: The line search algorithm (implemented in KINLineSearch) was unable to satisfy the beta-condition for MXNBCF + 1 nonlinear iterations (not necessarily consecutive), which may indicate the algorithm is making  poor progress ");
	   break;
   case 409  : strcpy(C2F(cha1).buf,"KIN_LINSOLV_NO_RECOVERY: The user-supplied routine psolve encountered a recoverable error, but the preconditioner is already current");
	   break;
   case 410  : strcpy(C2F(cha1).buf,"KIN_LINIT_FAIL: The linear solver initialization routine (linit) encountered an error.");
	   break;
   case 411  : strcpy(C2F(cha1).buf,"KIN_LSETUP_FAIL: The user-supplied routine pset (used to compute the preconditioner) encountered an unrecoverable error .");
	   break;
   case 412  : strcpy(C2F(cha1).buf,"KIN_LSOLVE_FAIL: Either the user-supplied routine psolve (used to to solve the preconditioned linear system) encountered an unrecoverable error, or the linear solver routine  (lsolve) encountered an error condition.");
	   break;
   case 413  : strcpy(C2F(cha1).buf,"KIN_SYSFUNC_FAIL: Error in the computing function. Please verify your model.");
	   break;
   case 414  : strcpy(C2F(cha1).buf,"KIN_FIRST_SYSFUNC_ERR: The system function failed at the first call. Please verify your model. There might be an error in the computing function, for example a function may be called with illegal inputs.");
	   break;
   case 415  : strcpy(C2F(cha1).buf,"KIN_REPTD_SYSFUNC_ERR: The system function failed repeatedly. Please verify your model. There might be an error in the computing function, for example a function may be called with illegal inputs");
	   break;
   case 416  : strcpy(C2F(cha1).buf,"KIN_NAN_ERR: The residual function returns NAN. Please verify your model: some functions might be called with illegal inputs.");
	   break;
