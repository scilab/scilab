 ...
 * Sets the rotation property of uicontrol imagerender 
 * 
...
#include "SetUiobjectRotate.hxx"

using namespace org_scilab_modules_gui_bridge;

int SetUiobjectRotate(sciPointObj* sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
  ...
  if (valueType == sci_matrix)
    {
      ...
      if (nbCol == 0 || nbRow ==0) /* Empty matrix value */
      ...
      else
        {
          ...
          pUICONTROL_FEATURE(sciObj)->doubleValue = new double[nbCol];
          for (K=0; K<nbCol; K++)
            {
