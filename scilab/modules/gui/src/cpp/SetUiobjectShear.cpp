...
#include "SetUiobjectShear.hxx"

using namespace org_scilab_modules_gui_bridge;

int SetUiobjectShear(sciPointObj* sciObj, size_t stackPointer, int valueType, int nbRow, int nbCol)
{
  ...
  if (valueType == sci_matrix)
    {
	  ...
	  /* Ensures that if user inputs only one variable for x coordinate in shear, it passes an extra 0 to shear function for y coordinate */
          pUICONTROL_FEATURE(sciObj)->doubleValue[nbCol] = 0;
        }
    }
  else
    {
