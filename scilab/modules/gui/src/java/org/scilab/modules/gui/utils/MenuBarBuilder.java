...
public final class MenuBarBuilder {
    ...
    /**
     * Create a Scilab menubar from data in a XML file using Scilab MVC
     * @param fileToLoad XML file to load
     * @param parentId the UID of the parent (used for menus)
     */
    public static void buildMenuBar(String fileToLoad, String parentId) {
        buildMenuBar(fileToLoad, parentId, 0);
    }
     ...
     * @param parentId the UID of the parent (used for menus)
     */
    public static void buildMenuBar(String fileToLoad, String parentId, int figureIndex) {
    ...
    private static class MenuBarConfigurationHandler implements InvocationHandler {
        ...
        public void addMenus(String parentId) {
            ...
            for (int i = 0; i < menus.getLength(); i++) {
                // Create the menu
                String menuId = GraphicController.getController().askObject(Type.UIMENU);
                ...
                if (submenu.getNodeName() == SEPARATOR) {
                    ...
                    while (callback != null) {
                        if (callback.getNodeName() == CALLBACK) {
                            ...
                            if (command != null && commandType != CallBack.UNTYPED) {
                                GraphicController.getController().setProperty(menuId, __GO_CALLBACK__, replaceFigureID(command));
        ...
        public void addSubMenuItem(String parentMenuItemId, Node node) {
            ...
            // Create the menu
            String subMenuItemId = GraphicController.getController().askObject(Type.UIMENU);
            ...
            while (callback != null) {
                if (callback.getNodeName() == CALLBACK) {
                    ...
                    if (command != null && commandType != CallBack.UNTYPED) {
                        GraphicController.getController().setProperty(subMenuItemId, __GO_CALLBACK__, replaceFigureID(command));
