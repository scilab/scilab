...
types::SparseBool* cwiseOp(types::Sparse const& op1, types::Sparse const& op2)
{
    ...
    {
        res= op2.isComplex()
            ? cwiseOp(*op1.matrixCplx, *op2.matrixCplx, Op<std::complex<double> >())
            /* TODO implement when Eigen provides coeff() for Sparse<>::cast<>
               cwiseOp(*op1.matrixCplx, op2.matrixReal->template cast<std::complex<double> >(), Op<std::complex<double> >());*/
            : 0;
    }
    else
    {
        res= op2.isComplex()
            ? 0/* TODO cf. above
                 cwiseOp(op1.matrixReal->template cast<std::complex<double> >(), *op2.matrixCplx, Op<std::complex<double> >())*/
            : cwiseOp(*op1.matrixReal, *op2.matrixReal, Op<double>());
    }
