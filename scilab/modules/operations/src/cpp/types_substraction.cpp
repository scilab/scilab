...
int SubstractSparseToSparse(Sparse* sp1, Sparse* sp2, GenericType** pSpRes)
{
     return (*pSpRes=sp1->substract(*sp2)) ? 0 : 1;
}
int SubstractDoubleToSparse(Double* d, Sparse* sp, GenericType** pDRes)
{
     /* substracting from a scalar 0. is a special case that preserves sparsness */
    if((d->size_get() == 1) && (d->real_get(0,0) == 0.) && ((!d->isComplex() || d->img_get(0,0) == 0.)))
    {
        Sparse* tmp(new Sparse(*sp));
        tmp->opposite();
        *pDRes= tmp;
        return 0;
    }
    ...
    sp->fill(tmp);
    return SubstractDoubleToDouble(&tmp, d, (Double**)pDRes);
}
int SubstractSparseToDouble(Sparse* sp, Double* d, GenericType** pDRes)
{
     /* substracting a scalar 0. is a special case that preserves sparsness */
     if((d->size_get() == 1) && (d->real_get(0,0) == 0.) && ((!d->isComplex() || d->img_get(0,0) == 0.)))
     {
         *pDRes= new Sparse(*sp);
         return 0;
    ...
    Double tmp(sp->rows_get(), sp->cols_get(), sp->isComplex());
    sp->fill(tmp);
    return SubstractDoubleToDouble( d, &tmp,  (Double**)pDRes);
}
