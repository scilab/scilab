...
struct string_matrix* create_string_matrix(int *_piKey, int arg)
{
  ...
  getVarAddressFromPosition(_piKey, arg, &varaddr);
  getMatrixOfString(_piKey, varaddr, &mat->rows, &mat->cols, NULL, NULL);
  size = mat->rows * mat->cols;
  lengths = (int *) MALLOC(size * sizeof(int));
  mat->strings = (char **) MALLOC((size * sizeof(char *)));
  getMatrixOfString(_piKey, varaddr, &mat->rows, &mat->cols, lengths, NULL);
  for (i = 0; i != size; ++i)
    mat->strings[i] = (char *)MALLOC((lengths[i] + 1) * sizeof(char));
  getMatrixOfString(_piKey, varaddr, &mat->rows, &mat->cols, lengths, mat->strings);
  FREE(lengths);
  return mat;
}
...
char *
create_string(int* _piKey, int arg)
{
  int rows, cols;
  char **strings;
  char *str;
  ...
  getVarAddressFromPosition(_piKey, arg, &varaddr);
  getMatrixOfString(_piKey, varaddr, &rows, &cols, NULL, NULL);
  if (rows != 1 || cols != 1)
    return NULL; /* not a mere string */
  lengths = (int *) MALLOC(sizeof(int));
  strings = (char **) MALLOC(sizeof(char *));
  getMatrixOfString(_piKey, varaddr, &rows, &cols, lengths, NULL);
  strings[0] = (char *)MALLOC((lengths[0] + 1) * sizeof(char));
  getMatrixOfString(_piKey, varaddr, &rows, &cols, lengths, strings);
  str = *strings;
  FREE(lengths);
  FREE(strings);
  return str;
