...
create_string_matrix(int arg)
{
  ...
  getVarAddressFromPosition(arg, &varaddr);
  getMatrixOfString(varaddr, &mat->rows, &mat->cols, NULL, NULL);
  size = mat->rows * mat->cols;
  lengths = (int *) MALLOC(size * sizeof(int));
  mat->strings = (char **) MALLOC((size * sizeof(char *)));
  getMatrixOfString(varaddr, &mat->rows, &mat->cols, lengths, NULL);
  for (i = 0; i != size; ++i)
    mat->strings[i] = (char *)MALLOC((lengths[i] + 1) * sizeof(char));
  getMatrixOfString(varaddr, &mat->rows, &mat->cols, lengths, mat->strings);
  FREE(lengths);
  return mat;
}
...
create_string(int arg)
{
  ...
  getVarAddressFromPosition(arg, &varaddr);
  getMatrixOfString(varaddr, &rows, &cols, NULL, NULL);
  if (rows != 1 || cols != 1)
    return NULL; /* not a mere string */
  lengths = (int *) MALLOC(sizeof(int));
  strings = (char **) MALLOC(sizeof(char *));
  getMatrixOfString(varaddr, &rows, &cols, lengths, NULL);
  strings[0] = (char *)MALLOC((lengths[0] + 1) * sizeof(char));
  getMatrixOfString(varaddr, &rows, &cols, lengths, strings);
  str = *strings;
  FREE(lengths);
  FREE(strings);
  return str;
