...
// Copyright INRIA
  ...
  if length(scs_mb)<=2 then return end
    //superblock should not inherit the context nor the name
    scs_mb(1).context=' ' 
    scs_mb(1).title(1)='Untitled'
	...
	scs_m(nn).graphics.pin(prt(k,2))=nnk+1
	...
	scs_m(nn).graphics.pout(prt(k,2))=nnk+1
	...
	scs_m(nn).graphics.pein(prt(k,2))=nnk+1
	...
	scs_m(nn).graphics.peout(prt(k,2))=nnk+1
      end
