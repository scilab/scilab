...
// B = prod(A,dim)
  ...
  if typeof(dim)=="cste" then
    if dim.value>size(A.dims) then
      tree.name="mtlb_prod"
      set_infos("Scilab prod() does not work when dim input argument is greater than number of dims of first rhs...",1)
      tree.lhs(1).dims=allunknown(A.dims)
    else
      tree.lhs(1).dims=A.dims
    ...
    tree.name="mtlb_prod"
    repl1=tree;repl1.name="prod";repl1.rhs=Rhs(A,dim)
    repl_poss(tree,..
