...
case 'set' then
  ...
  while %t do
    [ok,Bf,Br,Is,Vak,Tauf,Taur,Ccs,Cje,Cjc,Phie,..
     Me,Phic,Mc,Gbc,Gbe,Vt,EMinMax,exprs]=getvalue('Set PNP Transistor block parameters',..
	['Forward beta';..
	 'Reverse beta';..
...
case 'define' then
  ...
  gr_i=['if orient then';
        '   xx=orig(1)+[0, 8, 8, 10,10,20,30,20,10,20,32,20,10,11,13.5,10,10, 8 ,8]*(sz(1)/32);';
	'   yy=orig(2)+5+[30,30,17,17,30,17,17,17,30,42,42,42,30,35,32,30,42,42,30]*(sz(2)/70); ';
        'else';
        '   xx=orig(1)+[32, 24, 24, 22,22,12,2,12,22,12,0,12,22,21,18.5,22,22, 24 ,24]*(sz(1)/32);';
	'   yy=orig(2)+5+[30,30,17,17,30,17,17,17,30,42,42,42,30,35,32,30,42,42,30]*(sz(2)/70); ';
	'end';
	'xpoly(xx,yy);'
	'xfpoly(xx,yy)']
