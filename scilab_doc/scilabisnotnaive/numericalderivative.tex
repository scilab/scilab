...
As we can see, the \emph{derivative} command is based on 
floating point comprehension. Is it completely \emph{bulletproof} ?
Not exactly. 
...
The exact answer is $x_e=2. \times 10^{-100}$ so that the 
result does not have any significant digits.
...
$x$ is near 1. The behaviour near zero is the same.
